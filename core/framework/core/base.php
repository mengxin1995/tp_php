...
final class Base{
	...
	private static function parse_conf(&$setting_config){
		...
		$setting['shopnc_version'] = 'Copyright '.date('Y',time()).' <a href="http://shop.cuncuntu.com" target="_blank">村村兔</a> All rights reserved.<br />';
	...
	private static function cp(){
		...
		if (strpos(self::CPURL,'||') !== false){
			...
			header('location: http://zjcuntao.com');exit();
		}else{

...
//				}
		...
		//是否把图片传至图片服务器
		if($this->isremote && C('upload_service_enabled') == 1){
			//获取设置信息
			$upload_config = array(
				...
				'timeout'  => '90',//C('upload_service_timeout'),
			);
			...
			$file['savepath'] = DIR_UPLOAD.DS.$this->save_path;
			...
			//删除原图后直接返回,不再产生缩略图,缩略图由OSS直接提供,通过@!240的形式.前提是先定义好样式.
			//删除原图

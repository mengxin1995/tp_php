 ...
 * 文件上传类
 *
 * alphawu cuncuntu.com 2016.03.17
 *
 */
...
class Upload{
    ...
    private $config = array(
        ...
        'rootPath' => './public/attachment/', //保存根路径
        'savePath' => '', //保存路径
     ...
     */
    public function __construct($driver = '', $driverConfig = null){
    	/* 获取配置 */
        ...
        //引入上传引擎
        import('libraries.upload.'.$driver);
    	$this->setDriver(strtolower($driver), $driverConfig);
    ...
    private function setDriver($class, $config){
        ...
        if(!$this->uploader){
            Log::record("不存在上传驱动：".$config,Log::ERR);
        }
    }

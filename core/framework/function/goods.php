...
function thumb($goods = array(), $type = ''){
    ...
    if (empty($goods)){
        return UPLOAD_SITE_URL . DS . defaultGoodsImage($type);
    }
    ...
    $uri = C('upload_service_domain') . DS . DIR_UPLOAD . DS . ATTACH_GOODS . DS . $store_id . DS  .$goods['goods_image'];
    ...
    //TODO 这里到远端检查文件是否存在,这样会不会导致性能问题?? by alphawu ,2016.3.18
    //文件在远端存储
    if(C('upload_service_enabled') == 1){
        if(checkRemoteFile($uri)){
            return $uri .'@!' .$type;
            exit;
        }
    }
    ...
    $thumb_host = UPLOAD_SITE_URL . DS . ATTACH_GOODS;
    return $thumb_host . DS . $store_id . DS . $file;
}
/**
...
function cthumb($file, $type = '', $store_id = false) {
    ...
    //文件在远端存储
    if(C('upload_service_enabled') == 1){
        $uri = C('upload_service_domain') . DS . DIR_UPLOAD . DS . ATTACH_GOODS . DS . $store_id . DS . $file;
        if(checkRemoteFile($uri)){
            return $uri.'@!'.$type;
        }
    }
    ...
    if ( !file_exists(BASE_UPLOAD_PATH . DS . ATTACH_GOODS . DS . $store_id . DS . ($type == '' ? $file : str_ireplace('.', '_' . $type . '.', $file)) )) {
        return UPLOAD_SITE_URL.DS.defaultGoodsImage($type);
    }
    $thumb_host = UPLOAD_SITE_URL . DS . ATTACH_GOODS;
    return $thumb_host . DS . $store_id . DS . ($type == '' ? $file : str_ireplace('.', '_' . $type . '.', $file));
}
/**
 * 商品二维码
...
function goodsQRCode($goods_info) {
    if (!file_exists(BASE_UPLOAD_PATH . DS .  ATTACH_STORE  . DS .  $goods_info['store_id']  . DS .  $goods_info['goods_id'] . '.png' )) {
...
function storeQRCode($store_id) {
    if (!file_exists(BASE_UPLOAD_PATH . DS . ATTACH_STORE . DS . $store_id  . DS . $store_id . '_store.png')) {
        return UPLOAD_SITE_URL.DS.ATTACH_STORE.DS.'default_qrcode.png';
    }
...
function brandImage($image_name = '') {
    if ($image_name != '') {
        return UPLOAD_SITE_URL . DS . ATTACH_BRAND . DS . $image_name;
    }
    return UPLOAD_SITE_URL . DS . ATTACH_COMMON . DS . 'default_brand_image.gif';
}
...
function orderPaymentName($payment_code) {
    return str_replace(
            array('offline','online','alipay','tenpay','chinabank','predeposit','balance','sfpay','wxpay'),
            array('货到付款','在线付款','支付宝','财付通','网银在线','站内余额支付','充值卡支付','顺手付','微信支付'),
            $payment_code);
}
...
{
    //已经将文件都同步到远端,不需要检查了,2016.03.20 alphawu
    return true;

...
class member_orderControl extends mobileMemberControl {
    ...
    public function search_deliverOp(){
        ...
        $e_code ='zjcuntao_express'; //$express[$order_info['extend_order_common']['shipping_express_id']]['e_code'];
        $e_name ='诸暨村淘物流'; //$express[$order_info['extend_order_common']['shipping_express_id']]['e_name'];

        $deliver_info = $this->_get_express($e_code, $order_info['shipping_code'],$order_info);
    ...
    public function _get_express($e_code, $shipping_code,$order_info){
        ...
        if ($content['status'] != 200) { 
            ...
            $content='
{"message":"ok","nu":"888888","ischeck":"1","com":"zj","status":"200","condition":"F00","state":"3","data":[{ "time":"'.date('Y-m-d H:i:s',$order_info['delay_time']).'","context":"卖家已发货。","ftime":"'.date('Y-m-d H:i:s',$order_info['delay_time']).'" },{ "time":"","context":"","ftime":"" }]}';
            $content=json_decode($content);

        }
        else{
            array_push($content['data'],array('time'=>date('Y-m-d H:i:s',$order_info['delay_time']),'context'=>'卖家已发货','ftime'=>date('Y-m-d H:i:s',$order_info['delay_time'])));
        }

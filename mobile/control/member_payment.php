...
class member_paymentControl extends mobileMemberControl {
	...
	private function _api_pay($order_pay_info, $mb_payment_info) {
        ...
        if($mb_payment_info['payment_code']=='cuntaopay'){
            $payment_api = new $this->payment_code($mb_payment_info,$order_pay_info,$_GET['key']);
            $payment_api->submit();
        }elseif($mb_payment_info['payment_code']=='wxpay'){
            $payment_api = new $this->payment_code($mb_payment_info,$order_pay_info);
            $payment_api->submit();
        }elseif($mb_payment_info['payment_code']=='sfpay'){
            $payment_api = new $this->payment_code($mb_payment_info,$order_pay_info);
            $payment_api->submit();
        }
        else{

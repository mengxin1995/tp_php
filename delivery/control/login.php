...
class loginControl extends BaseAccountCenterControl{
    ...
    public function indexOp() {
        ...
        if (chksubmit()) {
            ...
            $condition = array();
            $condition['member_name'] = $_POST['dname'];
            $condition['member_passwd'] = md5($_POST['dpasswd']);
            $member_info = Model('member')->getMemberInfo($condition);
            if (!empty($member_info) && !empty($dp_info)) {

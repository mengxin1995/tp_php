...
class stat_goodsControl extends SystemControl{
    ...
    public function goods_saleOp(){
        ...
        if ($_GET['exporttype'] == 'excel'){
			...
			if (!is_numeric($_GET['curpage'])){
				$array = array();
				if ($countnum > self::EXPORT_SIZE ) {
					//如果记录数量多，则进行分页,显示下载链接
					//计算分页数$page
					$page = ceil($countnum / self::EXPORT_SIZE);
					for ($i = 1; $i <= $page; $i++) {
						$limit1 = ($i - 1) * self::EXPORT_SIZE + 1;
						$limit2 = $i * self::EXPORT_SIZE > $countnum ? $countnum : $i * self::EXPORT_SIZE;
						$array[$i] = $limit1 . ' ~ ' . $limit2;
					}
					Tpl::output('list', $array);
					Tpl::output('murl', 'index.php?act=stat_goods&op=goods_sale');
					Tpl::showpage('export.excel');
				}
				else
				{
					//如果记录数量小，则直接下载
					$goods_list = $model->statByStatordergoods($where, $field, 0, self::EXPORT_SIZE, $orderby, 'goods_id');
			...
			}else
			{
				...
				$goods_list = $model->statByStatordergoods($where, $field, 0, "{$limit1},{$limit2}", $orderby, 'goods_id');
				$this->createExcel($goods_list);
			}
		} else {
			//查询品牌
	...
	private function createExcel($data = array()){
		...
		foreach ((array)$data as $k=>$v){
			...
			$tmp[] = array('data'=>$v['goods_commonid']);
			$tmp[] = array('data'=>$v['store_name']);
			$tmp[] = array('data'=>$v['goodsnum']);
			$tmp[] = array('data'=>$v['ordernum']);
			$tmp[] = array('data'=>$v['goodsamount']);
			$excel_data[] = $tmp;
		}

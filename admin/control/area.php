...
class areaControl extends SystemControl{
	...
	public function area_addOp(){
		...
		$model_area = Model('area');
		$parent_id = $_GET['area_parent_id'];
		$area = $model_area->getAreaInfo(array('area_parent_id'=>$parent_id));
		...
		//获取所有子地区
		$deepl = "".(intval($deep)+1);
		$all_listl = area_cache::getCache('area',array('deep'=>$deepl));
		foreach($all_listl as $val){
			if($val['area_parent_id'] == $area_id){
				$area_listl[] = $val;
			}
		}
		//获取所有子地区
		$deepl = "".(intval($deepl)+1);
		$cache_datal = area_cache::getCache('area',array('deep'=>$deepl));
		if (is_array($cache_datal)){
			foreach ($cache_datal as $k => $v){
				foreach($area_listl as $key =>$val){
					if ($v['area_parent_id'] == $val['area_id']){
						$area_listl[] = $v;
					}
				}
		...
		if(is_array($area_listl)){
			$area_idl=array();
			foreach($area_listl as $key =>$val){
				$area_idl[] =$val['area_id'];
			}
			Tpl::output('area_idl',$area_idl);
		}
		...
		//获取所有上级地区y
		$deepf = "".(intval($deep)-1);
		$all_listf = area_cache::getCache('area',array('deep'=>$deepf));
		foreach($all_listf as $value){
			if($value['area_id'] == $parent_id){
				$area_listf[] = $value;
			}
		}

		//获取所有上级地区
		$deepf = "".(intval($deepf)-1);
		$cache_dataf = area_cache::getCache('area',array('deep'=>$deepf));
		if (is_array($cache_dataf)){
			foreach ($cache_dataf as $k => $v){
				foreach($area_listf as $key =>$val){
					if ($v['area_id'] == $val['area_parent_id']){
						$area_listf[] = $v;
					}
				}
			}
		...
		if(is_array($area_listf)){
			$area_idf=array();
			foreach($area_listf as $key =>$val){
				$area_idf[] =$val['area_id'];
			}
			Tpl::output('area_idf',$area_idf);
		}
		...
		if (chksubmit()){
			...
			/*增加是否允许自定义地址开始*/
			$insert_array['area_address'] 	= $_POST['area_address'];
			/*增加是否允许自定义地址结束*/
	...
	public function area_editOp(){
		...
		$area = $model_area->getAreaInfo(array('area_id'=>$area_id));
		$parent_id = $area['area_parent_id'];
		//父ID
		if($parent_id == 0){
			$deep = 1;
		}else{
		...
		if (chksubmit()){
			...
			$area_parent_id = $_POST['area_id'];
			//拿到所有子地区id
			$area_parent_idl=$_POST['area_idl'];
			if(!empty($area_parent_idl)){
				$data = array();
				$condition = array();
				foreach($area_parent_idl as $key =>$val){
					$data['area_address'] 	= $_POST['area_address'];
					$condition['area_id'] = $val;
					$result = $model_area->editArea($data, $condition);
				}
			}
			//拿到所有上级地区id
			$area_parent_idf=$_POST['area_idf'];
			if(!empty($area_parent_idf)){
				$data = array();
				$condition = array();
				foreach($area_parent_idf as $key =>$val){
					$data['area_address'] 	= '2';
					$condition['area_id'] = $val;
					$result = $model_area->editArea($data, $condition);
				}
			...
			/*增加是否允许自定义地址开始*/
		     $update_array['area_address'] 	= $_POST['area_address'];
			/*增加是否允许自定义地址结束*/

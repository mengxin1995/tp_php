...
class refundControl extends SystemControl{
	...
	public function export_step1Op(){
		...
		if (!is_numeric($_GET['curpage'])){
			...
			if ($count > self::EXPORT_SIZE ){	//显示下载链接
				...
				$data = $model_refund->getRefundListForExport($condition,self::EXPORT_SIZE);
	...
	private function createExcel($data = array()){
		...
		$excel_obj->addWorksheet($excel_obj->charset(L('exp_refund_order'),CHARSET));
		$excel_obj->generateXML($excel_obj->charset(L('exp_refund_order'),CHARSET).$_GET['curpage'].'-'.date('Y-m-d-H',time()));
	}
}

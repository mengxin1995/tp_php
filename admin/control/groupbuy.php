...
class groupbuyControl extends SystemControl{
    ...
    public function groupbuy_listOp(){
        ...
        $groupbuy_list = $model_groupbuy->getGroupbuyExtendList($condition, 10,'state asc,groupbuy_id desc');
    ...
    /**
     * 重启，根据老沈的要求，by zjcuntao
     */
    public function groupbuy_restartOp() {
        $groupbuy_id = intval($_POST['groupbuy_id']);

        $model_groupbuy = Model('groupbuy');
        $result = $model_groupbuy->restartGroupbuy($groupbuy_id);
        if($result) {
        	$this->log('重启抢购活动，抢购编号'.$groupbuy_id,null);
            showMessage(L('nc_common_op_succ'), '');
        } else {
            showMessage(L('nc_common_op_fail'), '');
        }

...
class orderControl extends SystemControl{
	...
	public function indexOp(){
		...
		/*订单金额区间搜索开始*/
		if(!empty($_GET['query_start_price'])){
			$price1	= $_GET['query_start_price'];
		}
		if(!empty($_GET['query_end_price'])){
			$price2	= $_GET['query_end_price'];
		}
		if ($price1 && $price2){
			$condition['goods_amount'] = array('between',array($price1,$price2));
		}elseif($price1){
			$condition['goods_amount'] = array('egt',$price1);
		}elseif($price2){
			$condition['goods_amount'] = array('elt',$price2);
		}
		/*订单金额区间搜索开始*/

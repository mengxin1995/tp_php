...
class rechargecardControl extends SystemControl
{
    ...
    public function add_cardOp()
    {
        ...
        if ($denomination > 50000) {
            showMessage('面额不能大于50000', '', 'html', 'error');
            return;
        }
        ...
        case '0':
			...
			$card_length = intval($_POST['card_length']);
			if ($card_length < 10 || $card_length > 32) {
                showMessage('长度只能是10-32直接的整数', '', 'html', 'error');
                exit;
            }
			$card_length = $card_length >9 ? $card_length : 32;//如果不大于10
			$card_length = $card_length >32 ? 32 : $card_length;//如果大于32
			
            while (count($snKeys) < $total) {
                $snKeys[$prefix . substr(md5(uniqid(mt_rand(), true)),0,$card_length)] = null;
            }
            break;

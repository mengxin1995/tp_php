...
class web_apiControl extends SystemControl {
	...
	public function focus_editOp() {
		...
		$web_list = $model_web_config->getWebList(array('site_id'=>$site_id,'web_page'=>'index_pic'));

	    $web_id = $web_list[0]['web_id'];
		...
		$site_list = $site_model->getSiteList(null,null);

		Tpl::output('site_list',$site_list);
		Tpl::output('site_id',$site_id);
	...
	public function sale_editOp() {
	    $model_web_config = Model('web_config');
		$site_id = empty($_GET['site_id'])?0:$_GET['site_id'];
		$web_list = $model_web_config->getWebList(array('site_id'=>$site_id,'web_page'=>'index_sale'));
		$web_id = $web_list[0]['web_id'];

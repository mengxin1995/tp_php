...
class goodsControl extends SystemControl{
     ...
     * 编辑商品锁定，
     */
    public function goods_lock_editOp() {
        //获取商品对应的goods_commonid
        $goods_id=$_GET['goods_id'];
        $goods_model=Model("goods");
        $goods_common_info=$goods_model->where(array("goods_id"=>$goods_id))->field('goods_commonid')->find();
        $goods_commonid=$goods_common_info['goods_commonid'];
        //获取商品锁状态
        $goods_common_model=Model('goods_common');
        $lock_type_info=$goods_common_model->where(array('goods_commonid'=>$goods_commonid))->field('goods_lock')->find();
        $lock_type=$lock_type_info['goods_lock'];

        if(isset($_GET['goods_id'])){
        Tpl::output('goods_id',$_GET['goods_id']);
        }

        Tpl::output('lock_type',$lock_type);
        Tpl::showpage('goods.lock');
    }

    /**
     * 保存商品锁定，
     * goods_lock  1全部锁定 2价格锁定 3库存锁定 4价格和库存锁定
     */
    public function goods_lock_saveOp() {
        //获取商品对应的goods_commonid
        $goods_id=$_GET['goods_id'];
        $goods_model=Model("goods");
        $goods_common_info=$goods_model->where(array("goods_id"=>$goods_id))->field('goods_commonid')->find();
        ...
        if($_GET['all_lock']==1){
            ...
            //如果不是all_lock
          if($_GET['price_lock']==1&&$_GET['stock_lock']==0){
              //价格锁定
              $update = array('goods_lock' =>2);
          }elseif($_GET['price_lock']==0&&$_GET['stock_lock']==1)
          {
              //库存锁定
              $update = array('goods_lock' =>3);
          }elseif($_GET['price_lock']==1&&$_GET['stock_lock']==1)
          {
              //价格和库存锁定
              $update = array('goods_lock' =>4);
          }else
          {
              //解除锁定
            ...
            //更新goods_common表中的goods_lock字段
            $goods_common_model=Model('goods_common');
            $goods_common_model->where(array('goods_commonid'=>$goods_commonid))->update($update);

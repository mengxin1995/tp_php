...
class mb_specialControl extends SystemControl{
    ...
    public function goods_listOp() {	
	   ...
	   $goods_id_list=$model_goods->getXianshiGoodsExtendIds($condition);		
        ...
        $condition['goods_name']=array(array('like','%'.$key[0].'%'),array('like','%'.$key[1].'%'),'and');
        /*$condition['goods_name'] = array('like', '%' . $_GET['keyword'] . '%');*/
		$goods_list = $model_goods->getGoodsOnlineList2($condition, 'goods_id,goods_name,goods_promotion_price,goods_image', 10);
        Tpl::output('goods_list', $goods_list);

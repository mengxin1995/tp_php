...
class specControl extends SystemControl {
	...
	public function specOp(){
		 ...
		 * 检索条件
		 */
		if (!empty($_GET['search_spec_name'])){
			$condition['sp_name'] = array('like',"%".$_GET['search_spec_name']."%");
		}
		
		
		//$spec_list	= $model_spec->specList(array('order'=>'sp_sort asc'), $page);
		
		$spec_list	= $model_spec->specListNew($condition, "*", 10);

		Tpl::output('search_spec_name',trim($_GET['search_spec_name']));

		Tpl::output('spec_list',$spec_list);
		//Tpl::output('page',$page->show());
		Tpl::output('page',$model_spec->showpage());

...
class uploadControl extends SystemControl{
	private $links = array(
		...
		array('url'=>'act=upload&op=service','lang'=>'upload_service'),
	...
	public function serviceOp(){
		if (chksubmit()){
			$obj_validate = new Validate();
			$obj_validate->validateparam = array(
				array("input"=>$_POST["upload_service_type"], "require"=>"true", "message"=>L('upload_servic_type_is_null')),
				array("input"=>$_POST["upload_service_host"], "require"=>"true", "message"=>L('upload_servic_host_is_null')),
				array("input"=>trim($_POST["upload_service_username"]), "require"=>"true", "message"=>L('upload_servic_username_is_null')),
				array("input"=>trim($_POST["upload_service_password"]), "require"=>"true", "message"=>L('upload_servic_password_is_null')),
				array("input"=>trim($_POST["upload_service_bucket"]), "require"=>"true", "message"=>L('upload_servic_bucket_is_null')),
				array("input"=>trim($_POST["upload_service_domain"]), "require"=>"true", "message"=>L('upload_servic_domain_is_null'))
			);
			$error = $obj_validate->validate();
			if ($error != ''){
				showMessage($error);
				...
				$model_setting = Model('setting');
				$result = $model_setting->updateSetting(array(
						'upload_service_enabled'	=> $_POST['upload_service_enabled'],
						'upload_service_type'		=> $_POST['upload_service_type'],
						'upload_service_host'		=> $_POST['upload_service_host'],
						'upload_service_username'	=> $_POST['upload_service_username'],
						'upload_service_password'	=> $_POST['upload_service_password'],
						'upload_service_bucket'		=> $_POST['upload_service_bucket'],
						'upload_service_domain'		=> $_POST['upload_service_domain']
					)
				);
		...
		Tpl::output('list_setting',$list_setting);
		//输出子菜单
		Tpl::output('top_link',$this->sublink($this->links,'service'));

		Tpl::showpage('upload.service');
	}

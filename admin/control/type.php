...
class typeControl extends SystemControl {
	...
	public function typeOp(){
		...
		$type_list	= $model_type->typeListNew($condition, "*", 10);
	...
	public function type_addOp(){
		...
		if (chksubmit()){
			...
			if(!empty($_POST['brand_id'])){
                ...
                if(!empty($_POST['brand_id2'])) {
                    $brand_array = explode(',', $_POST['brand_id2']);
                }
		...
		// 品牌列表,该功能转移到了brand.php的getBrandListOp
...
//		Tpl::output('brand_list', $b_list);
	...
	public function type_editOp(){
		...
		//品牌,该功能转移到了brand.php的getBrandListForEditOp，zjcuntao
//		$model_brand	= Model('brand');
//		/**
//		 * 检索条件
//		 */
...
//		}
//
//		$brand_list		= $model_brand->getBrandPassedList($condition);
//		$b_list = array();
//		if(is_array($brand_list) && !empty($brand_list)){
//			foreach($brand_list as $k=>$val){
//				$b_list[$val['class_id']]['brand'][$k] = $val;

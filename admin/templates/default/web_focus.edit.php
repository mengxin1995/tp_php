    ...
    <ul class="home-templates-board-style">
      <?php if(is_array($output['site_list'])){ ?>
        <?php foreach($output['site_list'] as $k => $v){ ?>
          <li class="orange <?php if($output['site_id'] == $v['site_id']){ ?> selected <?php } ?> "><a href="index.php?act=web_api&op=focus_edit&site_id=<?php echo $v['site_id'];?>"><em></em><i class="icon-ok"></i><?php echo $v['site_name'];?></a></li>
        <?php } ?>
      <?php } ?>
    </ul>
  </div>
    ...
    <form id="upload_screen_form" class="tab-content" name="upload_screen_form" enctype="multipart/form-data" method="post" action="index.php?act=web_api&op=screen_pic" target="upload_pic">
      ...
      <div class="full-screen-slides">
        ...
        <div class="add-focus"><a class="btn-add-nofloat" href="JavaScript:add_screen('pic');"><?php echo '图片调用';?></a>
            ...
            <?php } ?>
            <span class="s-tips"><i></i><?php echo '小提示：单击图片选中修改，拖动可以排序，添加最多不超过15个，保存后生效。';?></span></div>
      </div>

...
<div class="page">
  ...
  <form id="area_form" method="post">
	...
	<input type="hidden" name="have_area_parent_id" value="<?php echo $output['area_parent_id']?>" />
	  <?php if(isset($output['area_idl'])){ ?>
		  <?php foreach($output['area_idl'] as $key =>$val){ ?>
			  <input type="hidden" name="area_idl[]" value="<?php echo $val ?>" />
		  <?php }?>
	  <?php }?>
	  <?php if(isset($output['area_idf'])){ ?>
		  <?php foreach($output['area_idf'] as $key =>$val){ ?>
			  <input type="hidden" name="area_idf[]" value="<?php echo $val ?>" />
		  <?php }?>
	  <?php }?>
	<table class="table tb-type2">
	  <tbody>
		...
		<tr class="noborder">
		  ...
		  <td class="vatop tips">默认只有一级地区才需要填写大区域，目前全国几大区域有：华北、东北、华东、华南、华中、西南、西北、港澳台、海外。</td>
		</tr>
		...
		<tr class="noborder">
		  <td colspan="2" id="region">
			...
			<span></span>
			系统将根据所选择的上级菜单层级决定新增项的所在级，即选择上级菜单为“诸暨市”，则新增项为诸暨地区的下级区域，以此类推。</td>
		</tr>
		<?php }else{?>
		<tr>
		  <td colspan="2" class="required"><label for="area_name"><?php echo $lang['area_parent_id'];?>:</label></td>
		...
		<!--增加是否允许自定义地址开始-->
		<tr>
			<td colspan="2" class="required"><label><?php echo "是否允许自定义地址";?>:</label></td>
		</tr>
		<tr class="noborder">
			<td class="vatop rowform"><ul>
					<li>
						<input name="area_address" type="radio" value="0" <?php if($output['area_array']['area_address'] == '0'){ ?>checked="checked"<?php } ?>>
						<label>不允许</label>
					</li>
					<li>
						<input name="area_address" type="radio" value="1" <?php if($output['area_array']['area_address'] == '1'){ ?>checked="checked"<?php } ?>>
						<label>允许</label>
					</li>
				</ul></td>

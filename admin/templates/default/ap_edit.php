  ...
  <form id="link_form" enctype="multipart/form-data" method="post" name="form1">
    ...
    <table class="table tb-type2 nobdb">
      ...
      <tbody>
		...
		<!--增加分站开始-->
		<tr>
			<td colspan="2" class="required"><label><?php echo $lang['adv_index_site'];?>:</label></td>
		</tr>
		<tr class="noborder">
			<td class="vatop rowform "><ul>
					<?php if(is_array($output['site_list'])){ ?>
						<?php foreach($output['site_list'] as $key => $val){ ?>
							<li>
								<input type="radio" <?php if($v['site_id'] == $val['site_id']){ ?>checked="checked"<?php } ?> value="<?php echo $val['site_id'];?>" name="site_id" id="site_id<?php echo $val['site_id'];?>">
								<label for="site_id<?php echo $val['site_id'];?>"><?php echo $val['site_name'];?></label>
							</li>
						<?php } ?>
					<?php } ?>
				</ul></td>

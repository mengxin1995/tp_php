  ...
  <form id="navigation_form" method="post">
    ...
    <table class="table tb-type2">
      <tbody>
        ...
        <tr>
          <td colspan="2" class="required"><label>
            <label for="type"><?php echo $lang['navigation_index_site'];?>:</label>
            </label></td>
        </tr>
        <tr class="noborder">
          <td class="vatop rowform "><ul>
              <?php if(is_array($output['site_list'])){ ?>
                  <?php foreach($output['site_list'] as $k => $v){ ?>
                      <li>
                          <input type="radio" <?php if($output['navigation_array']['site_id'] == $v['site_id']){ ?>checked="checked"<?php } ?> value="<?php echo $v['site_id'];?>" name="site_id" id="site_id<?php echo $v['site_id'];?>">

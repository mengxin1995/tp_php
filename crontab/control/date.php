...
//         $this->_add_points();
        ...
        //将退款且已收货的订单更新为已取消 ，by zjcuntao
        $this->_order_refund_to_cancel();
    }
    ...
    /**
     * 已收货，并且退款的订单，更新为已取消,by zjcuntao
     */
    private function _order_refund_to_cancel() {
        ...
        $condition['refund_state'] = array('gt',1);//表示已经全额退款的订单
        $condition['add_time'] = array('lt',TIMESTAMP - ORDER_AUTO_CANCEL_DAY * self::EXE_TIMES);
        //分批，每批处理100个订单，最多处理5W个订单
        for ($i = 0; $i < 500; $i++){
            if ($_break) {
            ...
            if (empty($order_list)) break;
            foreach ($order_list as $order_info) {
                $result = $logic_order->changeOrderStateCancelFromRefund($order_info,'system','系统','实物订单将已退款的更新为已取消,by zjcuntao');
                if (!$result['state']) {
                    $this->log('实物订单将已退款的更新为已取消失败SN:'.$order_info['order_sn']); $_break = true; break;
                }
            }
        }
    }
            ...
            for ($i=0; $i<$countnum; $i+=100){//每次查询100条
                ...
                if ($orderlog_list){
                    ...
                    foreach ((array)$order_list_tmp as $k=>$v){
                        ...
                        //    $v['order_isvalid'] = 1;//退款取消的订单不纳入统计范围了。by zjcuntao alpha
            ...
            foreach ((array)$update_arr as $k=>$v){
                foreach ($v as $m_k=>$m_v){
                    ...
                    if ($statmember_info){
                        ...
                        $m_v['statm_orderamount'] = floatval($statmember_info['statm_orderamount']) + $m_v['statm_orderamount'];

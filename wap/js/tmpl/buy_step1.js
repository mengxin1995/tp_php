$(function () {
	...
	$.ajax({//提交订单信息
		...
		success: function (result) {
			...
			if (total_price <= Number(data.available_rc_balance) + Number(data.available_predeposit)) {
				$('.pre-deposit-wp').show();
				$('#wrapper-usecuntaopay').show();
				$('#cuntaopay_amount1').html(p2f(Number(data.available_rc_balance) + Number(data.available_predeposit)));
				...
				$('input[name=cuntaopay_amount]').val(Number(data.available_rc_balance) + Number(data.available_predeposit));
			}
			else {
				$('.pre-deposit-wp').show();
				$('#wrapper-usecuntaobpaynotenough').show();
				$('#cuntaopay_amount2').html(p2f(Number(data.available_rc_balance) + Number(data.available_predeposit)));
			}
			...
			//点击自提点显示
			$('#new-dlyp-button').click(function () {
				$('#area_delivery').show();
				$('#area_address').hide();
				$('.error-tips2').hide();
				$('.error-tips').hide();
	...
	$.ajax({//获取区域列表
		...
		success: function (result) {
			...
			$("select[name=delivery_prov]").append(prov_html);
		}
	});
	...
	/*自提点开始*/
	$("select[name=delivery_prov]").change(function () {//选择省县
		...
		$.ajax({
			...
			success: function (result) {
				...
				$("select[name=delivery_city]").html(city_html);
				$("select[name=delivery_region]").html('<option value="">请选择...</option>');
				$('#addr').hide();
				$('#dlyp').hide();
				$('#area_delivery').hide();
				$('#area_address').hide();
	...
	$("select[name=delivery_city]").change(function () {//选择乡镇
		var city_id = $(this).val();
		$.ajax({
			type: 'post',
			url: ApiUrl + '/index.php?act=zjcuntao&op=delivery_list',
			...
			success: function (result) {
				...
				for (var i = 0; i < data.list.length; i++) {
					region_html += '<option value="' + data.list[i].dlyp_id + '">' + data.list[i].dlyp_address_name + '，站长：' + data.list[i].dlyp_truename + '，电话：' + data.list[i].dlyp_mobile + '</option>';
				}
				$("select[name=delivery_region]").html(region_html);
			}
		});
		/*增加自定义地址开始*/
		if (city_id != '请选择...') {
			$.ajax({
				type:'post',
				url: ApiUrl + '/index.php?act=member_buy&op=add_ajax',
				data:{area_id:city_id,key:key},
				dataType:'json',
				success:function(result){
	...
	$(".save-address").click(function () {//更换收获地址
		var self = this;
		var selfPr;
		var intHot = $("input[name='zdyaddress']:checked").val();
		//获取address_id
		...
		if (address_id > 0) {//变更地址
			...
			if (intHot ==1) {
				$.sValid.init({//地址验证
					rules: {
						delivery_vtrue_name: "required",
						delivery_vmob_phone: "required",
						delivery_vprov: "required",
						delivery_vcity: "required",
						vaddress: "required"
					},
					messages: {
						delivery_vtrue_name: "required",
				...
				if ($.sValid()) {
					var index = $('select[name=delivery_prov]')[0].selectedIndex;
					var aa = $('select[name=delivery_prov]')[0].options[index].innerHTML;


					var true_name = $('input[name=delivery_true_name]').val();
					var mob_phone = $('input[name=delivery_mob_phone]').val();
					var tel_phone = $('input[name=delivery_tel_phone]').val();
					var city_id = $('select[name=delivery_city]').val();
					var address = $('input[name=vaddress]').val();

					var prov_index = $('select[name=delivery_prov]')[0].selectedIndex;
					var city_index = $('select[name=delivery_city]')[0].selectedIndex;
					var area_info = $('select[name=delivery_prov]')[0].options[prov_index].innerHTML + ' ' + $('select[name=delivery_city]')[0].options[city_index].innerHTML;
				...
				$.sValid.init({//地址验证
					rules: {
						...
						delivery_vprov: "required",
						delivery_vcity: "required",
						delivery_vregion: "required",
					},
					messages: {
						delivery_vtrue_name: "姓名必填！",
						delivery_vmob_phone: "手机号必填！",
						delivery_vprov: "省份必填！",
						delivery_vcity: "城市必填！",
						delivery_vregion: "自提点必填！",
					},
					callback: function (eId, eMsg, eRules) {
						if (eId.length > 0) {
							var errorHtml = "";
							...
							$("#error-tips2").html(errorHtml).show();
						} else {
							$("#error-tips2").html("").hide();
						}
					}
				});
				if ($.sValid()) {
					...
					var tel_phone = $('input[name=delivery_tel_phone]').val();
					var city_id = $('select[name=delivery_city]').val();
					var dlyp_id = $('select[name=delivery_region]').val();

					var prov_index = $('select[name=delivery_prov]')[0].selectedIndex;
					var city_index = $('select[name=delivery_city]')[0].selectedIndex;
					var region_index = $('select[name=delivery_region]')[0].selectedIndex;
					var area_info = $('select[name=delivery_prov]')[0].options[prov_index].innerHTML + ' ' + $('select[name=delivery_city]')[0].options[city_index].innerHTML + ' ' + $('select[name=delivery_region]')[0].options[region_index].innerHTML;

					//ajax 提交收货地址
					$.ajax({
						type: 'post',
						url: ApiUrl + '/index.php?act=member_address&op=delivery_add',
						data: {
							key: key,
							true_name: true_name,
							mob_phone: mob_phone,
	...
	$(".save-invoice").click(function () {//保存发票信息
		...
		if (inv_id > 0) {//选择发票信息
			...
			if (inv_title_select == 'company') {
				...
				var data = {
					key: key,
					inv_title_select: inv_title_select,
					inv_title: inv_title,
	...
	$('#usecuntaopay').click(function () {//验证密码切换
		if ($('#usecuntaopay').attr('checked')) {
			$('#pd').show();
		} else {
			$('#pd').hide();
		}
	...
	$('#buy_step2').click(function () {//提交订单step2
		...
		$.ajax({
			type:"GET",
			url:ApiUrl +"/index.php?act=member_address&op=getAddressLockType_ajax",
...
function  buy_step2(){
	...
	data.cuntao_pay = 0;
	var cuntaopay_amount = parseInt($('input[name=cuntaopay_amount]').val());

	if (cuntaopay_amount > 0 && $('#usecuntaopay').prop('checked')) { // 使用充值卡
		var passwd_verify = parseInt($('input[name=passwd_verify]').val());
		if (passwd_verify != 1) { // 验证密码失败
			$('.password_error_tip').show();
			$('.password_error_tip').html('请"使用"密码');
			return false;
		}
		data.cuntao_pay = 1;

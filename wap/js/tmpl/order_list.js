$(function(){
	...
	function initPage(page,curpage){
		$.ajax({
			...
			success:function(result){
                ...
                //站内支付订单
                $(".cuntaopay").click(cuntaopayOrder);
    ...
    //站内支付订单
    function cuntaopayOrder(){
        var key = $(this).attr("data-key");
        var pay_sn = $(this).attr("data-pay_sn");

        $.sDialog({
            content: '确定余额支付订单？',
            okFn: function() { cuntaopayOrderOk(key,pay_sn); }
        });
    }

    function cuntaopayOrderOk(key,pay_sn) {
        $.ajax({
            type:"post",
            url:ApiUrl+"/index.php?act=zjcuntao&op=cuntaopay",
            data:{pay_sn:pay_sn,key:key,code:'cuntaopay'},
            dataType:"json",
            success:function(result){
                if(result.datas && result.datas == 1){
                    initPage(page,curpage);

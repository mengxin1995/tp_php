...
<div class="ncc-form-default">
  ...
  <?php if(!empty($output['addr'])){ ?>
    ...
    <dl>
      ...
      <dd>
        <div id="region">
          <input type="text" class="text w500" maxlength="80" value="<?php echo $output['addr']['area_info']?>" readonly="readonly" style="border:none;background: transparent;"/>
          <input type="hidden" value="<?php echo $output['addr']['city_id']?>" name="city_id" id="city_id">
          <input type="hidden" name="area_id" id="area_id" class="area_ids" value="<?php echo $output['addr']['area_id']?>"/>
          <input type="hidden" name="area_info" id="area_info" class="area_names" value="<?php echo $output['addr']['area_info']?>"/>		  
        </div>
      </dd>
    </dl>
    <dl>
      <dt><i class="required">*</i>村淘站长：</dt>
      <dd>
        <input type="text" class="text w500" name="address" id="address" maxlength="80" value="<?php echo $output['addr']['address']?>" readonly="readonly" style="border:none;background: transparent;"/>
      </dd>
    </dl>	
    <dl>
  ...
  <?php } else {?>
    ...
    <dl>
      ...
      <dd>
        <input type="text" class="text w200" name="mob_phone" id="mob_phone" maxlength="15" value=""/>
        &nbsp;&nbsp;(或1)&nbsp;<?php echo $lang['cart_step1_phone_num'].$lang['nc_colon'];?>
        <input type="text" class="text w200" id="tel_phone" name="tel_phone" maxlength="20" value=""/>
      </dd>
    </dl>  
  <?php } ?>	
...
function submitAddAddr(){
    if ($('#addr_form').valid()){
        ...
        <?php if(!empty($output['addr'])){ ?> 
		$('#city_id').val(<?php echo $output['addr']['city_id']?>);
		<?php } else { ?>

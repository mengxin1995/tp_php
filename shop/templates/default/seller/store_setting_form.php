...
<div class="ncsc-form-default">
  <form method="post"  action="index.php?act=store_setting&op=store_setting" enctype="multipart/form-data" id="my_store_form">
    ...
    <dl>
      <dt><?php echo $lang['store_setting_store_location'].$lang['nc_colon']; ?></dt>
      <dd>
          <select name="store_location" id="store_location">
              <?php
              foreach ($output['province_list'] as $k=>$v){
                  echo "<option value='".$k."'";
                  if($k==$output['store_info']['province_id']){echo 'selected';}
                  echo " >".$v;
                  echo "</option>";
              }
              ?>
          </select>
        <p class="hint"><?php echo $lang['store_setting_store_location_tip'];?></p>
      </dd>
    </dl>

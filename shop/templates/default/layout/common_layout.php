...
$pc = $_GET['pc'];
if(!empty($pc)){
    $_SESSION['zjcuntao_pc']='true';//电脑版浏览标志，在mobile/control/index/indexOp里面清除
}

if(empty($_SESSION['zjcuntao_pc']) && ($ua == '' || preg_match($uachar, $ua))&& !strpos(strtolower($_SERVER['REQUEST_URI']),'wap'))
{
	global $config;
...
<?php require_once template("layout/index_popup") ?>
  ...
  <header class="public-head-layout wrapper">
	...
	<!--商品和店铺-->
      <ul class="tab">
        ...
        <li title="请输入您要搜索的订单号" act="order_list">订单</li>
      </ul>
      <form class="search-form" method="get" action="<?php echo SHOP_SITE_URL;?>" target="_blank">
...
<nav class="public-nav-layout">
  <div class="wrapper">
    ...
    <ul class="site-menu">
	  ...
	  <?php }?>
      <?php if (C('points_isuse') && C('pointshop_isuse') && 1>2){ ?>

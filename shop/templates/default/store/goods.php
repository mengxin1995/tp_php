...
<!--          <span class="ncs-goods-code-note"><i></i>客户端扫购有惊喜</span>-->
      ...
      <?php if($output['store_id']==344){//外部商品专用店铺 ?>
        <div class="ncs-buy">
          <div class="ncs-btn">
            <a data-type="0" biz-itemid="<?php echo $output['goods']['goods_barcode'];?>" data-tmpl="192×40" data-tmplid="625" data-rd="2" data-style="2" data-border="
1" href="#" target="_blank"></a>
          </div>
        </div>
      <?php } elseif($output['goods']['goods_state'] != 10 && $output['goods']['goods_verify'] == 1){?>
      <!-- S 促销 -->
      ...
      <div class="ncs-sale">
        ...
        <dl>
          <dt>服　　务：</dt>
          <dd>由 <a href="<?php echo urlShop('show_store', 'index', array('store_id' => $output['store_info']['store_id']), $output['store_info']['store_domain']);?>" title="进入店铺" ><span style="color:#D93600"><b><?php echo $output['store_info']['store_name']; ?></b></span></a> 发货并提供售后服务。<span id="store-free-time">18:00 前完成下单，预计(<span style="color:#D93600"><b>
    ...
    <div class="ncs-intro">
      <div class="content bd" id="ncGoodsIntro"> 
        ...
        <ul class="nc-goods-sort">
              <?php if(isset($output['goods']['brand_name']) && !empty($output['goods']['brand_name'])){echo '<li>'.$lang['goods_index_brand'].$lang['nc_colon'].$output['goods']['brand_name'].'</li>';}?>
              <?php if(is_array($output['goods']['goods_attr']) && !empty($output['goods']['goods_attr'])){?>
                <?php foreach ($output['goods']['goods_attr'] as $val){ $val= array_values($val);if($val[1]!='不限'){echo '<li>'.$val[0].$lang['nc_colon'].$val[1].'</li>'; }}?>
          <?php }?>
        </ul>
        <?php }?>
...
<?php if($output['goods']['goods_state'] == 1 && $output['goods']['goods_verify'] == 1 && $output['goods']['is_virtual'] == 0 && $output['store_id']!=344){//外部商品店铺不需要下面的脚本 ?>

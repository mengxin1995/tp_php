...
class member_addressControl extends BaseMemberControl{
    ...
    /**
     * 设定默认自提点,购买页面,选择一个地址就将其设置为默认地址,by alpha,2016.01.12
     */
	public function set_default_addressOp()
    {
        if(!empty($_GET['addr_id'])){
            ...
            $condition = array(
                'address_id'  => $_GET['addr_id']
            );
            $address_info = $model_address->getAddressInfo($condition);

            if(!empty($address_info) && $this->member_info['member_id'] == $address_info['member_id']){
                ...
                $model_address->editAddress($data,$condition);
	...
	private function profile_menu($menu_type,$menu_key='') {
		...
		switch ($menu_type) {
			case 'address':
				$menu_array = array(
				1=>array('menu_key'=>'address','menu_name'=>Language::get('nc_member_path_address_list'),	'menu_url'=>'index.php?act=member_address&op=address'));
				break;
		}

...
class store_goods_addControl extends BaseSellerControl {
    ...
    public function save_goodsOp() {
        if (chksubmit()) {
            ...
            $common_array['goods_position']      = $_POST['g_position'];
            ...
            if ($common_id) {
                ...
                if (is_array($_POST['spec'])) {
                    foreach ($_POST['spec'] as $value) {
                        ...
                        $goods['goods_position']     = $value['stock_position'];
                        ...
                        $tmp_insert['goods_image']      = $common_array['goods_image'];
			 ...
			 // 生成商品二维码
                    ...
                    $goods['goods_position']     = $common_array['goods_position'];
    ...
    public function save_imageOp(){
        if (chksubmit()) {
            ...
            if ($common_id <= 0 || empty($_POST['img'])) {
                ...
                Model('goods')->delGoodsImages(array('goods_commonid' => $common_id));
            }
            $model_goods = Model('goods');
            // 保存
            $insert_array = array();
    ...
    public function image_uploadOp() {
        ...
        if(C('upload_service_enabled') == 1){
            $uri = C('upload_service_domain'). DS . DIR_UPLOAD . DS . ATTACH_GOODS . DS . $_SESSION['store_id'] . DS . $img_path .'@info';
            ...
            // 取得图像大小
            list($width, $height, $type, $attr) = getimagesize(BASE_UPLOAD_PATH . DS . ATTACH_GOODS . DS . $_SESSION['store_id'] . DS . $img_path);
        }

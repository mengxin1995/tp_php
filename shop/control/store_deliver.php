...
//		}
	 ...
	 * 批量发货
	 */
	public function plsendOp(){
		$orderid = substr($_COOKIE['array_str'], 1, strlen($_COOKIE['array_str']) - 1);
		if (strlen($orderid) <= 0) {
		...
		foreach($order_aid as $k=>$v) {
			...
			Tpl::output('order'.$k, $order_info);

			try {
				$order_model->beginTransaction();
				$data = array();
				$order_sn = $order_info['order_sn'];
				$order_info = $order_model->getOrderInfo(array('order_sn'=>$order_sn),array('order_common','member','order_goods'));
				//
				...
				//
				$data = array();
				$data['shipping_code']  = ZJ.$order_sn;
				$data['order_state'] = 30;
				...
				$update = $order_model->addOrderLog($data,$condition);
				if (!$update) {
					throw new Exception('操作失败');
				}
		...
		showMessage('批量设置发货成功','','html','succ');
	}
	/**
	 * 编辑收货地址
	 * @return boolean
	...
	public function get_expressOp(){
		/*
		$url='http://wuliu.cuncuntu.com/index.php?g=&m=Order&a=ss&order_id=1';
        import('function.ftp');
		...
		*/
		...
		preg_match('/ZJ(?P<digit>\d+)/', $_GET['shipping_code'], $matches);
		$url='http://wuliu.cuncuntu.com/index.php?g=&m=Order&a=ss&order_id='.$matches['digit'];
		import('function.ftp');
		$content = dfsockopen($url);
	 ...
	 * 运单打印
	 */
	public function waybill_print_oldOp() {
		$order_id = intval($_GET['order_id']);
		if($order_id <= 0) {
			showMessage(L('param_error'));
		}
		...
		if(isset($_GET['store_waybill_id'])){
			$store_waybill_info = $this->_getCurrentWaybill($store_waybill_list, $_GET['store_waybill_id']);
		}


		//如果get中有$_GET['expresstype']，通过该值查找对应的store_waybill_id
		if(isset($_GET['expresstype'])&&!isset($_GET['store_waybill_id']))
		{
			//如果物流公司是村村通物流
			if($_GET['expresstype']=='村村通物流')
			{
				$this->waybill_printOp();
				exit;
			}
			//根据物流公司名称查找express_id
			$model_express = Model('express');
			$express_id=$model_express->getExpressIDByName($_GET['expresstype']);
			//根据express_id，store_id查找store_waybill_id
			$store_waybill_id=$model_store_waybill->getStoreWaybillIDByExpressIDAndStoreId($express_id,$_SESSION['store_id']);
			if($store_waybill_id!=null)
			{
		...
		if(isset($waybill_info['waybill_data']))
		{
		...
		//输出订单的sn号，便于生成二维码或者条形码 alphawu
		$waybill_info['order_sn']=$order_info['order_sn'];
	...
	public function waybill_printOp() {
		...
		if(!empty($order_info) && !empty($store_info)){
			$my_order['order_sn'] = $order_info['order_sn'];
			$my_order['shipping_code'] = $order_info['shipping_code'];
			$my_order['reciver_name'] = $order_info['extend_order_common']['reciver_name'];
			$my_order['buyer_name'] = $order_info['buyer_name'];
			$my_order['buyer_phone'] = $order_info['extend_order_common']['reciver_info']['phone'];
			$my_order['address'] = $order_info['extend_order_common']['reciver_info']['address'];
			$my_order['phone'] = empty($order_info['extend_order_common']['reciver_info']['mob_phone'])?$order_info['extend_order_common']['reciver_info']['tel_phone']:$order_info['extend_order_common']['reciver_info']['mob_phone'];
			$my_order['store_name'] = $store_info['store_name'];
			$my_order['store_phone'] = $store_info['store_phone'];
			$my_order['store_address'] = $store_info['store_address'];
			$my_order['station_name'] = $order_info['extend_order_common']['reciver_info']['station_name'];
		}
		else{
			$my_order[]=null;
			showMessage('无法打印，请检查订单或店铺信息是否完整', urlShop('seller_center', 'index'), '', 'error');
		}
		Tpl::output('order', $my_order);
		Tpl::showpage('waybill.printnew', 'null_layout');
	}

	/**
	 * 批量运单打印开始
	 */
	public function waybill_plprintOp() {
		...
		foreach($order_aid as $k=>$v) {
			...
			Tpl::output('order'.$k, $my_order);
		}
		Tpl::showpage('waybill.printpl', 'null_layout');
	}
	/**
	 * 批量运单打印结束
	 */
	...
	public function checkitemOp() {
		...
		$ary = explode(',',$_SESSION['array_str']);
		if($chk == 'checked'){
			if(!in_array($val,$ary)) array_push($ary, $val);
		}else{
			unset($ary[array_search($val,$ary)]);
		}
		$_SESSION['array_str'] = implode(',',$ary);

...
class store_albumControl extends BaseSellerControl {
    ...
    public function replace_image_uploadOp() {
        ...
        if(C('upload_service_enabled') == 1){
            $uri = C('upload_service_domain'). DS . DIR_UPLOAD . DS . ATTACH_GOODS . DS . $_SESSION['store_id']  . DS . $apic_info['apic_cover'] .'@info';
    ...
    public function image_uploadOp() {
		...
		if(C('upload_service_enabled') == 1){
            $uri = C('upload_service_domain'). DS . DIR_UPLOAD . DS . ATTACH_GOODS . DS . $store_id . DS . $pic .'@info';
            $json = file_get_contents($uri);

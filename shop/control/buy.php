...
class buyControl extends BaseBuyControl {
    ...
    public function buy_step1Op() {
        ...
        //输出运费商品信息,2016.01.12
        Tpl::output('fee_goods_id_list', $result['fee_goods_id_list']);
...
//        var_dump($result);exit;
    ...
    public function payOp() {
        ...
        if ($pay_amount_online > 0) {
            ...
            if (!empty($payment_list)) {
                //unset($payment_list['predeposit']);//使用站内余额支付 by zjcuntao
    ...
    public function pd_payOp() {
		...
		$model_payment = Model('payment');
        $condition = array();
        $condition['payment_code'] = array('not in',array('offline','predeposit','cuntaopay'));
    ...
    public function change_addrOp() {
     ...
     public function add_addrOp(){
		...
		$addr = $model_addr->getAddressInfo($condition,$order);
    ...
    public function add_ajaxOp() {
            ...
            exit(json_encode(array('area_info'=>$area_info)));
    }
    /**
     * 添加自提点型收货地址
     */
    public function add_deliveryOp() {
        if (chksubmit()) {
            $info = Model('delivery_point')->getDeliveryPointOpenInfo(array('dlyp_id'=>intval($_POST['dlyp_id'])));
            if (empty($info)) {
                exit(json_encode(array('state'=>false,'msg'=>'该自提点不存在')));
            }
            $data = array();
            $data['member_id'] = $_SESSION['member_id'];
            $data['true_name'] = $_POST['true_name'];
            ...
            if (!$result){
                exit(json_encode(array('state'=>false,'msg'=>'保存失败')));
            }
            ...
            Tpl::showpage('buy_address.delivery_add','null_layout');
        }
    }
    /**
     * 展示自提点列表
    ...
    public function delivery_listOp() {
        ...
        $condition = array();
        $area_id = isset($_GET['area_id']) ? intval(trim($_GET['area_id'])) : 0 ;
        ...
        //如果是站长，则只检索站长对应的自提点的信息
        if($_SESSION['is_station']=='1'){
            $condition = null;
            $condition['dlyp_name'] = $_SESSION['member_name'];
        }
        $list = $model_delivery->getDeliveryPointOpenList($condition,5);
        Tpl::output('show_page',$model_delivery->showpage());
        Tpl::output('list',$list);
        Tpl::showpage('buy_address.delivery_list','null_layout');
    }
	/**
	 * 加载买家发票列表，最多显示10条
	 *
    ...
    public function cuntaopayOp(){
        ...
        $condition = array();
        $condition['member_id'] = $_SESSION['member_id'];
        $result = $member_model->getMemberInfo($condition);
        ...
        Tpl::showpage('cuntaopay');
    }

    public function cuntaopayOKOp(){

        if (empty($_POST['password'])) return ;
        $buyer_info	= Model('member')->getMemberInfoByID($_SESSION['member_id']);
        $order_list_needpay = Model('order')->getOrderList(array('pay_sn'=>$_POST['pay_sn']));

        $logic_buy_1 = Logic('buy_1');

        if ($buyer_info['member_paypwd'] == '' || $buyer_info['member_paypwd'] != md5($_POST['password'])) return ;
        ...
        if($pay_amount>$avaliable_amount){
            showMessage('余额不足,支付失败','index.php?act=member_order','html','error');
            return;
        }

        //使用村淘站内支付
        if (!empty($_POST['cuntao_pay'])) {
...
//        }
        ...
        showMessage('支付完成','index.php?act=member_order','html','success');
    }
}

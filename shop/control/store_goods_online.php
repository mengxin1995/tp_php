...
class store_goods_onlineControl extends BaseSellerControl {
    ...
    public function edit_goodsOp() {
        ...
        // 取得商品规格的输入值
        $goods_array = $model_goods->getGoodsList($where, 'goods_id,goods_marketprice,goods_price,goods_storage,goods_serial,goods_storage_alarm,goods_position,goods_spec,goods_barcode');
        $sp_value = array();
        if (is_array($goods_array) && !empty($goods_array)) {
            ...
            foreach ( $goods_array as $k => $v ) {
                ...
                if (!empty($a)) {
                    ...
                    $sp_value ['i_' . $id . '|stock_position'] = $v['goods_position'];
     ...
     * ajax获取商品锁定状态
     */
    public function getGoodsLockType_ajaxOp()
    {
        $commonid=$_GET['commonid'];
        //获取商品锁定状态
        $goods_common_model=Model('goods_common');
        $goods_lock_info=$goods_common_model->where(array('goods_commonid'=>$commonid))->find();
        //获取商品goods_lock状态
        $goods_lock=$goods_lock_info['goods_lock'];
    ...
    public function edit_save_goodsOp() {
        ...
        //验证商品锁定状态是否改变，如果改变且字段与数据库不一致，报错
        $goods_common_model=Model('goods_common');
        $goods_lock_info=$goods_common_model->where(array('goods_commonid'=>$common_id))->find();
        //获取商品goods_lock状态
        $goods_lock=$goods_lock_info['goods_lock'];
        if($goods_lock==2||$goods_lock==4)
        {
           //获取商品价格
           $goods_price=$goods_lock_info['goods_price'];
           if($_POST['g_price']!=$goods_price)
           {
               showDialog('操作失败:商品价格已被锁定', urlShop('store_goods_online', 'index'),'confirm','','');
           }
        }
        if($goods_lock==3||$goods_lock==4)
        {
            //获取商品库存
            $goods_storage_info=$goods_common_model->table('goods')->where(array('goods_commonid'=>$common_id))->find();
            $goods_storage=$goods_storage_info['storage'];
            if($_POST['g_storage']!=$goods_storage)
            {
                showDialog('操作失败，商品库存已被锁定', urlShop('store_goods_online', 'index'),'confirm','','');
            }
        }
        ...
        $update_common['goods_position']    = $_POST['g_position'];
        ...
        if (is_array ( $_POST ['spec'] )) {
            foreach ($_POST['spec'] as $value) {
                $goods_info = $model_goods->getGoodsInfo(array('goods_id' => $value['goods_id'], 'goods_commonid' => $common_id, 'store_id' => $_SESSION['store_id']));
                if (!empty($goods_info)) {
                    ...
                    $update['goods_position']   = $value['stock_position'];
		    ...
		    // 生成商品二维码
                    ...
                    $insert['goods_position']     = $value['stock_position'];
            ...
            if (!empty($goods_info)) {
                ...
                $update['goods_position']     = $update_common['goods_position'];

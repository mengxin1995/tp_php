...
class predepositControl extends BaseMemberControl {
	...
	public function recharge_addOp(){
        ...
        $data['pdr_rate'] = $member_info['member_rate'];
        $data['pdr_real_amount'] = $pdr_amount*$member_info['member_rate'];
    ...
    public function rcb_log_listOp()
    {
		...
		if (!empty($_GET['description'])) {
			$condition['description'] = array('like', '%'.$_GET['description'].'%');
		}
		$list = $model_rcb->getRcbLogList($condition,20,'*','id desc');
        //信息输出
        self::profile_menu('log', 'rcb_log_list');
        Tpl::output('show_page', $model_rcb->showpage());
        Tpl::output('list', $list);

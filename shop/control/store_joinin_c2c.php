...
class store_joinin_c2cControl extends BaseHomeControl {
	...
	public function __construct() {
        ...
        Tpl::output('html_title',C('site_name').' - '.'个人入驻');
    ...
    private function step3_save_valid($param) {
        ...
        $obj_validate->validateparam = array(
            ...
            array("input"=>$param['settlement_bank_account_number'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"30","message"=>"支付宝账号不能为空且必须小于30个字"),
        );
        $error = $obj_validate->validate();
        if ($error != ''){
            showMessage($error);

...
class searchControl extends BaseHomeControl {
    ...
    public function indexOp() {
        ...
        if (is_array($indexer_ids)) {
            ...
            $what = array();
            $what['member_id'] = $_SESSION['member_id'];
            $member = $model_member->getMemberInfo($what);
            $model_store = Model('store');
            if($member['wholesale'] == 0) {
                foreach($goods_list as $k=>$v) {
                    $where = array();
                    $where['store_id'] = $v['store_id'];
                    $store = $model_store->getStoreInfo($where);
                    if($store['wholesale'] == 1) {
                        unset($goods_list[$k]);
                    }
                }
            }
        ...
        if (!empty($goods_list)) {
            ...
            foreach ($goods_list as $key => $value) {
                ...
                //将关键字置红
                if ($search_keyword){
                    $goods_list[$key]['goods_name_highlight'] = str_replace($search_keyword,'<span style="color:#f00;">'.$search_keyword.'</span>',$value['goods_name']);
                } else {
		...
		if(count($goods_list)<self::PAGESIZE-5 && (empty($_GET['curpage']) || $_GET['curpage']<2)){
			//获取淘宝客数据
			$this->taobaoke();
            //$this->taobaoke_new();
		}
		...
		if(2>1 && $_GET['keyword'] != ''){
			...
			$model_search_log = Model('search_log');
			$model_search_log->addsearchLog($param);			
		}
...
//             Log::record('search\auto_complete'.$e->getMessage(),Log::RUN);
	...
	/**
	 * 获取淘宝客数据,2015.11.05新增，使用API来获取淘宝数据
	*/
	private function taobaoke_new(){
		...
		if($cate_id>0){
			$model_goods_class = Model('goods_class');
			$nav_link_list = $model_goods_class->getGoodsClassNav(intval($_GET['cate_id']));
			if(count($nav_link_list)>1){				
				for($i=0;$i<count($nav_link_list)-1;$i++){
					$keyword .= $nav_link_list[$i]['title'].' ';
					if($i>0)break;//这里只获取最后两个
				}
			}
		}
		$keyword=urlencode($keyword);	
		
		$content=F('tbk/'.$d.md5($keyword));
		if($content==null){
			//$tb_url='http://ai.taobao.com/search/getItem.htm?__ajax__=1&pid=mm_13859000_8490947_29744223&unid=&page=1&pageSize=60&key='.$keyword.'&sort=biz30day&tmall=true';
            $tb_url = BASE_SITE_URL.DS.'tbksdk'.DS.'tbk.php?type=&key='.$keyword;
			import('function.ftp');
			$content = dfsockopen($tb_url);
			F('tbk/'.$d.md5($keyword),$content);
		}
		...
		if($content!=null){
			...
			{
                $url = SHOP_SITE_URL.DS.'index.php?act=getitem&op=goods&id='.$val['num_iid'];

				$info['goods_id']=$val['num_iid'];

				$info['goods_name'] =$val['title'];
				//$info['goods_name']  = str_replace("&lt;em&gt;", "", $info['goods_name']);
				//$info['goods_name']  = str_replace("&lt;/em&gt;", "", $info['goods_name']);
				//$info['goods_name']  = str_replace("&lt;", "<", $info['goods_name']);
				//$info['goods_name']  = str_replace("&gt;", ">", $info['goods_name']);
				$info['pic_url'] =$val['pict_url'];
				$info['coupon_price']= $val['zk_final_price'];
				$info['price']= $val['reserve_price'];
				$info['click_url']= $url;
				$result['item_list'][]=$info;
			}
			$taobaoke_item_list = $result['item_list'];
			if(is_array($taobaoke_item_list) && !empty($taobaoke_item_list)){
				foreach ($taobaoke_item_list as $key => $vol) {
					$taokeitem[$key] = $vol;
				}
			}
			Tpl::output('taobaoke_item_list',$taokeitem);
		}
	}
	/**
	 * 获取淘宝客数据
	*/
	private function taobaoke(){
		$keyword=$_GET['keyword'];
		$d=date('ymd',time());
		$cate_id=intval($_GET['cate_id']);
		if($cate_id>0){
		...
		$content = json_decode($content,true);
		if($content!=null){
			$arr=($content['result']['auction']);
			//var_dump($arr);
			$info = array();
			foreach($arr as $key=>$val)
			{
				$info['goods_id']=$val['itemId'];
				$info['store_id']=$val['sellerId'];
				$info['goods_salenum']=$val['biz30Day'];
				$info['store_name']=$val['nick'];
				$info['evaluation_count']=$val['saleCount'];

				$info['goods_name'] =$val['description'];
				//$info['goods_name']  = str_replace("&lt;em&gt;", "", $info['goods_name']);
				//$info['goods_name']  = str_replace("&lt;/em&gt;", "", $info['goods_name']);
				$info['goods_name']  = str_replace("&lt;", "<", $info['goods_name']);
				$info['goods_name']  = str_replace("&gt;", ">", $info['goods_name']);
				$info['pic_url'] =$val['originalPicUrl'];
				$info['num_iid']= $val['itemId'];
				$info['commentNickName'] = $val['commentNickName'];
				$info['comment'] = $val['comment'];
				$info['cu'] = $val['itemLocation'];
				$info['coupon_price']= $val['realPrice'];
				$info['price']= empty($val['price'])?$val['realPrice']:$val['price'];
				$info['volume'] = $val['saleCount'];
				$info['click_url']= $val['clickUrl'];
				$info['type']= $val['userType'];

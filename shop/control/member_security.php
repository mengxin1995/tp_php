...
class member_securityControl extends BaseMemberControl {
     ...
     * 手机号码验证-发送验证码
     */
    public function send_validate_mobileOp() {
        $obj_validate = new Validate();
        $obj_validate->validateparam = array(
            array("input"=>$_GET["mobile"], "require"=>"true", 'validator'=>'mobile',"message"=>'请正确填写手机号码'),
        );
        ...
        $message = ncReplaceText($tpl_info['content'],$param);
        $sms = new Sms();
        if(!isset($_COOKIE['send_times'])) {
            $expire = time()+1800;
            setcookie(send_times,1,$expire);
            setcookie(send_times_expire,$expire,$expire);
            $result = $sms->send($_GET["mobile"],$message);
        } else {
            if($_COOKIE['send_times'] < 2) {
                setcookie(send_times,2,$_COOKIE['send_times_expire']);
                $result = $sms->send($_GET["mobile"],$message);
            } else {
                exit(json_encode(array('state'=>'false','msg'=>'由于运营商限制，30分钟内只能发送两次验证码，请稍后再发送')));
            }
        }
        if ($result) {
            $data = array();

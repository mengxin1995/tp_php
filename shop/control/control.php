...
class Control{
    ...
    protected function showLayout() {
        ...
        //分站 alphawu 2015.11.22 优化         修正导航栏显示不是同一站点的问题
        $mode_site = Model('site');
        if(isset($_GET['site_id'])){
            $site_id =substr($_GET['site_id'],0,10);
            $site = $mode_site->getOneSite($site_id);
            if(!$site){
                $site_id = 0;
            }
            setNcCookie('site_id',$site_id,30*24*3600);
        }
        ...
        $site = $mode_site->getOneSite($site_id);
        //这样的赋值在control.php里面的showLayout函数也有,showLayout先执行,这里重写一次,保证显示的结果是当前站点
        Tpl::output('current_site',$site);
    ...
    /**
     * 输出页面顶端分站信息 by alpha 2015.11.22
     */
    protected function showSite(){
        $mode_site = Model('site');
        $site_id = cookie('site_id');
        if(empty($site_id)){
            $site_id = 1;
        ...
        $site_list = $site_model->getSiteList2($site_id);

        Tpl::output('site_list',$site_list);
    }
    /**
    ...
    private function filter_nav($nav_list){
    	foreach ($nav_list as $key=>$nav){
    		if($nav['nav_is_close']=='1' || ($nav['site_id']!='0' && $nav['site_id']!=cookie('site_id'))){//根据设定的site_id来显示菜单 alphawu
    			unset($nav_list[$key]);
    		}
    	}
    	return $nav_list;
    ...
    private function _getMenuList() {
        $menu_list = array(
                ...
                'serv' => array('name' => '客户服务', 'child' => array(
                        ...
                        'member_logic'      => array('name' => '物流车辆', 'url'=>urlShop('member_logic', 'index')),
...
class BaseSellerControl extends Control {
    ...
    private function _getMenuList() {
        $menu_list = array(
            ...
            'message' => array('name' => '消息', 'child' => array(
                array('name' => '管理消息', 'act'=>'admin_msg', 'op'=>'index'),
                array('name' => '客服设置', 'act'=>'store_callcenter', 'op'=>'index'),
                array('name' => '系统消息', 'act'=>'store_msg', 'op'=>'index'),
                array('name' => '客服记录查询', 'act'=>'store_im', 'op'=>'index'),
            )),
            'account' => array('name' => '账号', 'child' => array(
                array('name' => '账户余额', 'act'=>'predeposit', 'op'=>'pd_log_list','target'=>'_blank'),

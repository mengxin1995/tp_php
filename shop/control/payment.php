...
class paymentControl extends BaseHomeControl{
	...
	private function _api_pay($order_info, $payment_info) {
    	...
    	if($payment_info['payment_code'] == 'chinabank' ||$payment_info['payment_code'] == 'cuntaopay'||$payment_info['payment_code'] == 'sfpay' ) {
    		$payment_api->submit();
            ...
            } else if (array_key_exists('pdr_id', $order_info)) {//预存款
                //Tpl::output('order_list',$order_info['order_list']);
                Tpl::output('args','buyer_id='.$_SESSION['member_id'].'&pdr_id='.$order_info['pdr_id']);
            } else {
    ...
    public function wxpay_notifyOp() {
		...
		//app支付的商户号跟公众号和扫码不一样,需要特殊处理,alphawu 2016.01.09
		$xml = $GLOBALS['HTTP_RAW_POST_DATA'];
		$r=	json_decode(json_encode(simplexml_load_string($xml, 'SimpleXMLElement', LIBXML_NOCDATA)), true);

		if($r['trade_type']=='APP'){
			$payment_info['payment_config']['appid']=C('wx_appid');
			$payment_info['payment_config']['mchid']=C('wx_mchid');
			$payment_info['payment_config']['key']=C('wx_appkey');

			new wxpay($payment_info,array());
		}else{
			new wxpay($result['data'],array());
		}

...
class goodsControl extends BaseGoodsControl {
					...
					//用此编号取数据，检测是否这次活动的订单商品。
					 ...
					 if($groupbuy_info)
					 {
                         ...
                         foreach($order_goods_list as $order_goods){
                             if($order_goods['promotions_id']==$promotions_id) {//只有是同一个团购的，才累加购买数量 by zjcuntao
                                 $total += $order_goods['goods_num'];
                             }
                     }
    ...
    public function salelogOp() {
        ...
        $goods_detail = $model_goods->getGoodsDetail($goods_id);
        $goods_info = $goods_detail['goods_info'];
        $goods_marketprice=$goods_info['goods_price']>0?$goods_info['goods_price']:$goods_info['goods_marketprice'];
        Tpl::output('marketprice',$goods_marketprice);
    ...
    //导出商品销售清单，by wangguoying 2015.09.30，临时用一下。
    public function exportOp() {
        $goods_id	 = intval($_GET['goods_id']);
        if ($_GET['vr']) {
            $model_order = Model('vr_order');
            $sales = $model_order->getOrderAndOrderGoodsSalesRecordList(array('goods_id'=>$goods_id), '*', 2000);
        } else {
            $model_order = Model('order');
            $sales = $model_order->getOrderAndOrderGoodsSalesRecordList(array('order_goods.goods_id'=>$goods_id), 'order_goods.*, order.buyer_name, order.add_time,order.order_sn,order.payment_time,order.order_state,order.refund_state', 2000);
        }
        $this->createExcel($sales,$goods_id);
    }
    /**
     * 生成excel
     *
     * @param array $data
     */
    private function createExcel($data = array(),$goods_id){
        ...
        //header
        $excel_data[0][] = array('styleid'=>'s_title','data'=>'rec_id');
        $excel_data[0][] = array('styleid'=>'s_title','data'=>'order_id');
        $excel_data[0][] = array('styleid'=>'s_title','data'=>'order_sn');
        ...
        $excel_data[0][] = array('styleid'=>'s_title','data'=>'购买价格');
        $excel_data[0][] = array('styleid'=>'s_title','data'=>'买家ID');
        $excel_data[0][] = array('styleid'=>'s_title','data'=>'购买时间');
        $excel_data[0][] = array('styleid'=>'s_title','data'=>'付款时间');
        $excel_data[0][] = array('styleid'=>'s_title','data'=>'order_state[订单状态：0(已取消)10(默认):未付款;20:已付款;30:已发货;40:已收货;]');
        $excel_data[0][] = array('styleid'=>'s_title','data'=>'refund_state[退款状态:0是无退款,1是部分退款,2是全部退款]');
        //data
        foreach ((array)$data as $k=>$v){
            $tmp = array();
            $tmp[] = array('data'=>$v['rec_id']);
            $tmp[] = array('data'=>$v['order_id']);
            $tmp[] = array('data'=>$v['order_sn']);
            $tmp[] = array('data'=>$v['goods_name']);
            $tmp[] = array('data'=>$v['goods_serial']);
            $tmp[] = array('data'=>$v['goods_barcode']);
            $tmp[] = array('format'=>'Number','data'=>ncPriceFormat($v['goods_price']));
            $tmp[] = array('data'=>$v['goods_num']);
            $tmp[] = array('format'=>'Number','data'=>ncPriceFormat($v['goods_pay_price']));
            $tmp[] = array('data'=>$v['buyer_name']);
            $tmp[] = array('data'=>date('Y-m-d H:i:s',$v['add_time']));
            $tmp[] = array('data'=>date('Y-m-d H:i:s',$v['payment_login_time']));
        ...
        $excel_obj->addWorksheet($excel_obj->charset('商品销量表_'.$goods_id,CHARSET));
        $excel_obj->generateXML($excel_obj->charset('商品销量表_'.$goods_id,CHARSET).$_GET['curpage'].'-'.date('Y-m-d-H',time()));
    }

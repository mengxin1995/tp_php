...
class member_connectControl extends BaseMemberControl {
	public function __construct() {
		 ...
		 * 读取语言包
		 */
		Language::read('member_member_qqconnect,member_member_sconnect,member_member_weixinconnect');
	}
	/**
	 * 微信帐号绑定
	 */
	public function weixinbindOp(){
		...
		$weixin_fans_info = $model_weixin_fans->getFansInfoByMemberID($this->member_info['member_id']);

		Tpl::output('weixin_fans_info',$weixin_fans_info);
	...
	public function weixinunbindOp(){
		...
		if ($_POST['is_editpw'] == 'yes'){
			...
			$obj_validate->validateparam = array(
					array("input"=>$_POST["new_password"],		"require"=>"true","validator"=>"Length","min"=>6,"max"=>20,"message"=>Language::get('member_weixinconnect_password_null')),
					array("input"=>$_POST["confirm_password"],	"require"=>"true","validator"=>"Compare","operator"=>"==","to"=>$_POST["new_password"],"message"=>Language::get('member_weixinconnect_input_two_password_again')),
			);
		...
		$edit_state = $model_weixin_fans->weixinunbind($_SESSION['member_id']);



		if(!$edit_state) {
			showMessage(Language::get('member_weixinconnect_password_modify_fail'),'html','error');
		}

		session_unset();
		session_destroy();
		showMessage(Language::get('member_weixinconnect_unbind_success'),'index.php?act=login&ref_url='.urlencode('index.php?act=member_connect&op=weixinbind'));
	}
	...
	private function profile_menu($menu_key='',$array=array()) {
		...
		$menu_array = array(
			...
			3=>array('menu_key'=>'weixin_bind',	'menu_name'=>$lang['nc_member_path_weixin_bind'],	'menu_url'=>'index.php?act=member_connect&op=weixinbind'),
		);
		if(!empty($array)) {
			$menu_array[] = $array;
		}

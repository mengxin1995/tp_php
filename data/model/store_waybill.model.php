...
class store_waybillModel extends Model{
    ...
    //找到返回store_waybill_id，没找到则返回null
    public function getStoreWaybillIDByExpressIDAndStoreId($express_id=null,$store_id=null)
    {
        if($express_id!=null&&$store_id!=null)
        {
            $condition=array();
            $condition['express_id']=$express_id;
            $condition['store_id']=$store_id;
            $store_waybill_info=$this->where($condition)->field('store_waybill_id')->find();
            return $store_waybill_info['store_waybill_id'];
        }
        else
        {
            return null;

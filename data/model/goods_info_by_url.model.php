...
class goods_info_by_urlModel{
    ...
    private function get_store_goods_info_by_url($url) {
        ...
        if(intval($goods_id) > 0) {
            ...
            $common_info= $model->getGoodeCommonInfoByID($goods_commonid);
            $model_xianshi_goods = Model('p_xianshi_goods');
            $xianshi_goods_info = $model_xianshi_goods->getXianshiGoodsInfoByGoodeID(intval($goods_id));
            if(!empty($goods_info)&&!empty($xianshi_goods_info)&&$xianshi_goods_info['state']==1) {
                $result = array();
                $result['result'] = 'true';
                $result['id'] = intval($goods_id);
                $result['url'] = $url;
                $result['price'] = $xianshi_goods_info['xianshi_price'];
                $result['storeid'] = $goods_info['store_id'];
                $result['title'] = $common_info['goods_name'];
                $result['img'] = $goods_info['goods_image'];
                $result['image'] = thumb($goods_info, 240);
                $result['type'] = 'store';
                return $result;
            }else if(empty($xianshi_goods_info)&&!empty($goods_info)){
                $result = array();
                $result['result'] = 'true';
                $result['id'] = intval($goods_id);
                $result['url'] = $url;

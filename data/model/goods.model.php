...
class goodsModel extends Model{
    ...
    /*大后台手机专题要求能搜索处于下架状态的商品*/
    public function getGoodsOnlineList2($condition, $field = '*', $page = 0, $order = 'goods_id desc', $limit = 0, $group = '', $lock = false, $count = 0) {
        $condition['goods_verify']  = self::VERIFY1;
        return $this->getGoodsList($condition, $field, $group, $order, $limit, $page, $lock, $count);
    }

    /**
     * 在售商品SKU列表,为了抢购而添加的。老沈要求,by zjcuntao
     *
     * @param array $condition 条件
     * @param string $field 字段
     * @param string $group 分组
     ...
     * @return array
     */
    public function getGoodsOnlineListForPromotion($condition, $field = '*', $page = 0, $order = 'goods_id desc', $limit = 0, $group = '', $lock = false, $count = 0) {
        //$condition['goods_state']   = self::STATE1;
        $condition['goods_verify']  = self::VERIFY1;
        return $this->getGoodsList($condition, $field, $group, $order, $limit, $page, $lock, $count);
    }	
    ...
    public function getGoodsCommonListForPromotion($condition, $field = '*', $page = 10, $type) {
        if ($type == 'groupbuy') {
            ...
            $condition['is_fcode']      = 0;
            $condition['is_presell']    = 0;
            //$condition['goods_state']   = self::STATE1;//为了抢购而添加的。老沈要求,by zjcuntao
            //$condition['goods_state']   =array('in',self::STATE1.','.self::STATE0.','.self::STATE10);
            $condition['goods_verify']  = self::VERIFY1;
        }
    ...
    /**
     * 更新商品主图数据,goods_images表
     * @param array $update
     * @param array $condition
     * @return boolean|unknown
     */
    public function editGoodsImages($update, $condition) {
        $result = $this->table('goods_images')->where($condition)->update($update);
        return $result;
    }

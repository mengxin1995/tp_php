...
class memberModel extends Model {
    ...
    public function createSession($member_info = array(),$reg = false) {
		...
		$_SESSION['is_station'] 	= $member_info['member_is_station'];//session里面增加是否是站长的标示
		...
		//session里面增加微信的openid,wxopenid,2015.12.12
		$model_weixin_fans = Model('weixin_fans');

		$weixin_fans = $model_weixin_fans->getFansInfoByMemberID($member_info['member_id']);

		if(!empty($weixin_fans)){
			$_SESSION['wxopenid'] = $weixin_fans['openid'];
			$_SESSION['unionid'] = $weixin_fans['unionid'];
		}
		//
    ...
    public function register($register_info) {
		...
		if(is_array($check_member_email) and count($check_member_email)>0 and $register_info['client'] != 'wap') {
     ...
     * 注册,for zjcuntao api
     */
    public function registerforzjcuntao($register_info) {
		// 注册验证
		$obj_validate = new Validate();
		$obj_validate->validateparam = array(
		array("input"=>$register_info["username"],		"require"=>"true",		"message"=>'用户名不能为空'),
		array("input"=>$register_info["password"],		"require"=>"true",		"message"=>'密码不能为空'),
		array("input"=>$register_info["member_paypwd"],		"require"=>"true",		"message"=>'支付密码不能为空'),
		array("input"=>$register_info["password_confirm"],"require"=>"true",	"validator"=>"Compare","operator"=>"==","to"=>$register_info["password"],"message"=>'密码与确认密码不相同'),
		array("input"=>$register_info["email"],			"require"=>"true",		"validator"=>"email", "message"=>'电子邮件格式不正确'),
		);
		$error = $obj_validate->validate();
		...
		//真实姓名也注册进去，用于手机端自动注册 by zjcuntao
		if(!empty($register_info['member_truename'])){
		...
		//支付密码注册进去，用于手机端自动注册 by zjcuntao
		if(!empty($register_info['member_paypwd'])){
		...
		//添加邀请人(推荐人)会员积分 by 33hao.com
		$member_info['inviter_id']		= $register_info['inviter_id'];
		$insert_id	= $this->addMemberforzjcuntao($member_info);
		if($insert_id) {
		    //添加会员积分
			if (C('points_isuse')){
				Model('points')->savePointsLog('regist',array('pl_memberid'=>$insert_id,'pl_membername'=>$register_info['username']),false);
	 ...
	 * @return	array 数组格式的返回结果
	 */
	public function addMemberforzjcuntao($param) {
		if(empty($param)) {
			return false;
		}
		try {
	...
	public function getOneMemberStat($member_id){

		$condition = array();
		$condition['statm_memberid'] = $member_id;

		//return $this->Table('stat_member')->field('sum(statm_ordernum) as order_num,sum(statm_orderamount) as order_amount,sum(statm_goodsnum) as order_goodsnum')->where($condition)->group('statm_memberid')->select();
		return $this->Table('stat_member')->field('sum(statm_ordernum) as order_num,sum(statm_orderamount) as order_amount,sum(statm_goodsnum) as order_goodsnum')->where($condition)->group('statm_memberid')->select();

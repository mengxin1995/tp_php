...
class statModel extends Model{
    ...
    public function dealwithSearchTime($search_arr){
        ...
        //时间区间
        if(!$search_arr['search_custom_start']){
            $search_arr['search_custom_start'] = date('Y-m-d', time()- 86400);
        }
        $search_arr['custom']['search_custom_start'] = strtotime($search_arr['search_custom_start']);
        if(!$search_arr['search_custom_end']){
            $search_arr['search_custom_end'] = date('Y-m-d', time()- 86400);
        }
        $search_arr['custom']['search_custom_end'] = strtotime($search_arr['search_custom_end']);
		//天
	...
	public function getStarttimeAndEndtime($search_arr){
        if($search_arr['search_type'] === 'last_thirty_days') {
        ...
        if($search_arr['search_type'] === 'custom') {
            $stime = $search_arr['custom']['search_custom_start'];
            $etime = $search_arr['custom']['search_custom_end']+ 86400 - 1;
        }

...
class predepositModel extends Model {
	...
	//充值卡充值接口，for zjcuntao api
    public function addRechargeCardforzjcuntao($sn,$memberId,$memberName)
    {
        if ($memberId < 1 || !$memberName) {
			output_error("当前登录状态为未登录，不能使用充值卡");
            exit;
        }
     ...
     * @param string $order
     */
    public function getRcbLogList($condition = array(), $pagesize = '', $fields = '*', $order = '', $limit = '') {
        return $this->table('rcb_log')->where($condition)->field($fields)->order($order)->limit($limit)->page($pagesize)->select();
    }
    /**
     * 变更充值卡余额
     *
    ...
    public function changeRcb($type, $data = array())
    {
        ...
        case 'transfer':
            $available = -$amount;
            $desc = '内部转账: 转给了账号：' . $data['receiver_user'].',转账原因：'.$data['reason'].'<br/>操作人：'.$data['admin_name'];
            break;
    ...
    public function changePd($change_type,$data = array()) {
        ...
        switch ($change_type){
            case 'order_receive':
                ...
                $data_log['lg_desc'] = '订单买家确认收货，收到货款。订单号: '.$data['order_sn'];
            ...
            case 'transfer':
                $data_log['lg_av_amount'] = $data['amount'];
                $data_log['lg_desc'] = '内部转账：来自用户: '.$data['pay_user'].',转账原因：'.$data['reason'].'<br/>操作人：'.$data['admin_name'];
                $data_log['lg_admin_name'] = $data['admin_name'];
                $data_pd['available_predeposit'] = array('exp','available_predeposit+'.$data['amount']);
                ...
                $data_msg['desc'] = $data_log['lg_desc'];
                break;
            case 'order_receive_cancel'://买家确认收货后又退款，从卖家账号扣款
                $data_log['lg_av_amount'] = -$data['amount'];
                $data_log['lg_desc'] = '订单买家取消订单，扣除货款。订单号: '.$data['order_sn'];
                $data_pd['available_predeposit'] = array('exp','available_predeposit-'.$data['amount']);
		...
		///修正支付宝充值部分时候会加倍的BUG		
		$c=$this->table('pd_log')->where(array('lg_type'=>'recharge','lg_desc'=>$data_log['lg_desc']))->count();//在线充值已经有记录了，不记录第二次
		if($c>0){exit();}
		///end 修正支付宝充值部分时候会加倍的BUG by zjcuntao.com
        //var_dump($data);var_dump($data_pd);exit;

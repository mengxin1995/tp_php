...
class web_configModel extends Model{
	 ...
	 * @return array 数组格式的返回结果
	 */
	public function getWebInfo($web_id){
		$condition = array();
		$condition['web_id'] = $web_id;
		$result = $this->table('web')->where($condition)->order('web_sort')->find();
		return $result;
	}
	...
	public function updateWebHtml($web_id = 1,$style_name = 'orange'){
		...
		if(!empty($code_list) && is_array($code_list)) {
			...
			//根据$web_id,查询一下web_page,不能仅仅根据id来进行判断

			$web_info = $this->getWebInfo($web_id);
			$web_page = $web_info['web_page'];
	 ...
	 *
	 */
	public function getWebHtml($web_page = 'index',$site_id=0,$update_all = 0){
		$web_array = array();
		$web_list = $this->getWebList(array('web_show'=>1,'site_id'=>$site_id,'web_page'=> array('like',$web_page.'%')));
		//var_dump($web_list);//exit;

...
class groupbuyModel extends Model{
     ...
     *
     */
    public function getMeiRiYiPin() {

        //获取今天的抢购信息。
        $condition_today = array();
        $condition_today['state'] = self::GROUPBUY_STATE_NORMAL;
        $condition_today['start_time'] = array('between',(string)(TIMESTAMP-12*60*60).','.(string)(TIMESTAMP+12*60*60));//开始时间在当前时间前后12个小时之内的。
        $condition_today['groupbuy_name'] =array('like','%每日一品%');
        $result_today = $this->getGroupbuyExtendList($condition_today, null, 'recommended desc', '*', 1);

        //获取7天之前的抢购信息
        $condition_7pre = array();
        $condition_7pre['state'] = array('in',self::GROUPBUY_STATE_NORMAL.','.self::GROUPBUY_STATE_CLOSE);
        $condition_7pre['start_time'] = array('between',(string)(TIMESTAMP-7*24*60*60).','.(string)(TIMESTAMP-12*60*60));//开始时间在当前时间前7天的。
        $condition_7pre['groupbuy_name'] =array('like','%每日一品%');
        $result_7pre = $this->getGroupbuyExtendList($condition_7pre, null, 'recommended desc ,groupbuy_id  desc', '*', 7);

        //获取7天之后的抢购信息
        $condition_7next = array();
        $condition_7next['state'] = array('in',self::GROUPBUY_STATE_NORMAL.','.self::GROUPBUY_STATE_CLOSE);
        $condition_7next['start_time'] = array('between',(string)(TIMESTAMP).','.(string)(TIMESTAMP+7*24*60*60));//开始时间在当前时间后7天的。
        $condition_7next['groupbuy_name'] =array('like','%每日一品%');
        $result_7next = $this->getGroupbuyExtendList($condition_7next, null, 'recommended desc, groupbuy_id asc', '*', 7);

        //return array_merge($result_today,$result_7pre,$result_7next);
        return $result_7next;


    }
    /**
     * 获取每日一品,手机端使用 by zjcuntao
     *
     */
    public function getMeiRiYiPinMobile() {
    ...
    /*
     * 重启某个抢购，根据老沈的要求 ，by zjcuntao
     * @param int $groupbuy_id
     * @return bool
     *
     */
    public function restartGroupbuy($groupbuy_id) {
        // 商品需要锁定
        $groupbuy_info = $this->getGroupbuyInfoByID($groupbuy_id);
        ...
        $return = $this->editGroupbuy($update, $condition);
        if ($return) {
            $this->_lockGoods($groupbuy_info['goods_commonid']);
        }
        return $return;
    }
    ...
    public function getGroupbuyExtendInfo($groupbuy_info) {
        ...
        //是否允许重新启动，管理员删除，并且结束时间小于当前时间的，才允许重启，by zjcuntao
        if($groupbuy_info['state'] == self::GROUPBUY_STATE_CANCEL && $groupbuy_info['end_time']>time()) {
            $groupbuy_info['restartable'] = 1;
        } else {
            $groupbuy_info['restartable'] = 0;
        }

...
class refund_returnModel extends Model{
	...
	public function editOrderRefund($refund) {
		...
		if ($refund_id > 0) {
    	    ...
    	    try {
				...
				//将已经确认收货，然后又退款的订单，货款从卖家账号里面扣除
				//1.先判断该订单是否是已经确认收货的订单；

				if($order['order_state']=='40'){
					//2.根据store_id找到卖家的member_id.
					//3.执行扣款操作
					$model_store = Model('store');
					$store_info = $model_store->getStoreInfoByID($order['store_id']);

					$log_array = array();
					$log_array['member_id'] = $store_info['member_id'];
					$log_array['member_name'] = $store_info['member_name'];
					$log_array['order_sn'] = $order['order_sn'];
					$log_array['amount'] = $refund['refund_amount'];//退预存款金额

					$state = $model_predeposit->changePd('order_receive_cancel', $log_array);//
				}
	 ...
	 * @return array
	 */
	public function getRefundListForExport($condition = array(), $limit = '') {
	    $condition['refund_type'] = '1';//类型:1为退款,2为退货
		$result = $this->table('refund_return')->where($condition)->limit($limit)->order('refund_id desc')->select();
		return $result;
	}
	 ...
	 * @return array
	 */
	public function getReturnListForExport($condition = array(), $limit = '') {
	    $condition['refund_type'] = '2';//类型:1为退款,2为退货
		$result = $this->table('refund_return')->where($condition)->limit($limit)->order('refund_id desc')->select();
		return $result;
	}

...
class addressModel extends Model {
     ...
     * @param array $condition
     */
    public function getLastAddressInfo($condition = array(), $order = 'is_default desc,address_id desc') {
        return $this->getAddressInfo($condition, $order);
    }
            ...
            if (!empty($dlyp_new_list)) {
                foreach ($address_list as $k => $v) {
                    ...
                    $address_list[$k]['address'] = '（'.$dlyp_info['dlyp_address_name'].') '.$dlyp_info['dlyp_address']
                        .'，站长：'.$dlyp_info['dlyp_truename']. '，电话：'.trim($dlyp_info['dlyp_mobile'].'，'.$dlyp_info['dlyp_telephony'],'，');
                }
            }
        }
        return $address_list;

...
class storeModel extends Model {
     ...
     * @return array
	 */
    public function getStoreList($condition, $page = null, $order = '', $field = '*', $limit = '100000') {
    ...
    public function getSendMsgList($condition, $pagesize = '', $field = '*', $order = 'store_id desc', $limit = '', $master = false)
    {
        $list = $this->table('store')->field($field)->where($condition)->page($pagesize)->order($order)->limit($limit)->master($master)->select();
     ...
     * @return array
     */
    public function getComList($condition, $pagesize = '', $field = '*', $order = 'com_id asc', $limit = '', $master = false)
    {
        $list = $this->table('monitor_cominfo')->field($field)->where($condition)->page($pagesize)->order($order)->limit($limit)->master($master)->select();
        if (empty($list)) return array();
        return $list;
    }

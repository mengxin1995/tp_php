...
final class area_Cache {
	...
	private static function getAreaCache($param) {
		...
		if (is_array($result)) {
			foreach ($result as $k => $v) {
				...
				/*增加允许自定义地址开始*/
				$tmp.= "\t\t'area_address'=>'" . $v['area_address'] . "',\r\n";
				/*增加允许自定义地址结束*/
				$tmp.= "\t),\r\n";
			}
		}
	...
	public static function updateAreaArrayJs()
	{
		...
		//SELECT area_parent_id FROM 33hao_area GROUP BY area_parent_id ORDER BY area_parent_id ASC
		//SELECT * FROM 33hao_area ORDER BY area_deep ASC,area_sort ASC,area_id ASC;
		$param = array();
		$param['table'] = "area";

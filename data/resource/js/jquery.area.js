			...
			$.ajax({
				...
				success: function(data){
					...
					for(var i = 0; i < data.length; i++){
						...
						src += "<td class='w60'><input type='checkbox' name='check_area_id[]' value='"+data[i].area_id+"' class='checkitem'>";
						//图片
						...
						/*增加允许自定义地址开始*/
						if(data[i].area_address =='0'){
							src += "<td class='w60'>"+"不允许"+"</td>";
						}else if (data[i].area_address =='1'){
							src += "<td class='w60'>"+"允许"+"</td>";
						}else if (data[i].area_address =='2'){
							src += "<td class='w60'>"+"部分允许"+"</td>";
						}
						//src += "<td class='w60'>"+data[i].area_address+"</td>";
						/*增加允许自定义地址结束*/
						//操作
						src += "<td class='w115'>";

...
//         if (empty($pd_payment_info)) return;
	...
	public function makeOrderSn($pay_id) {
		...
		return (date('y',time()) % 9+1) . sprintf('%09d', $pay_id) . sprintf('%02d', $num);
	}
    ...
    public function getReciverAddr($address_info = array()) {
        if (intval($address_info['dlyp_id'])) {
            $reciver_info['phone'] = trim(empty($address_info['mob_phone']) ? $address_info['tel_phone'] : $address_info['mob_phone']);//trim($address_info['dlyp_mobile'].($address_info['dlyp_telephony'] ? ','.$address_info['dlyp_telephony'] : null),',');
            ...
            $reciver_info['dlyp'] = 1;
            $reciver_info['station_name']=$address_info["dlyp_truename"];
            $reciver_info = serialize($reciver_info);
            $reciver_name = $address_info['true_name'];//.' '.trim($address_info['mob_phone'].($address_info['tel_phone'] ? ','.$address_info['tel_phone'] : null),',');
        } else {
    ...
    private function _getGoodsOnlineInfo($goods_id,$quantity) {
        ...
        $new_array['goods_serial'] = $goods_info['goods_serial'];//订单商品表增加一个字段 by alphawu 2015.9.25
        $new_array['goods_barcode'] = $goods_info['goods_barcode'];//订单商品表增加一个字段 by alphawu 2015.9.29
    ...
    public function getGroupbuyInfo(& $goods_info = array()) {
		...
		//$goods_model=Model('order');
		  ...
		  {
              //如果upper_limit =0的时候是不限制购买数量的 by alphawu 2015.09.24
              $tnum=intval($groupbuy_info['upper_limit'])>0?intval($groupbuy_info['upper_limit'])-intval($ordergoods):intval($goods_info['goods_num']);//-intval($goods_info['goods_num']);
              if($tnum<=0)
              {$goods_info=null;}
              //return;
              else{
    ...
    private function _getOnlineCartList($cart_list) {
        ...
        foreach ((array)$cart_list as $key => $cart_info) {
            ...
            if (in_array($cart_info['goods_id'],array_keys($goods_online_array))) {
                ...
                $cart_list[$key]['goods_barcode'] = $goods_online_info['goods_barcode'];

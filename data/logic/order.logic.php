...
class orderLogic {
     ...
     * @return array
     */
    public function changeOrderStateCancelFromRefund($order_info, $role, $user = '', $msg = '') {
        try {
            $model_order = Model('order');
            $model_order->beginTransaction();
            $order_id = $order_info['order_id'];
    ...
    public function changeOrderStateReceive($order_info, $role, $user = '', $msg = '') {
        try {
			...
			if(1){
				...
				$data_pd = array();				
				$data_pd['member_id'] = $store_info['member_id'];
				$data_pd['member_name'] = $store_info['member_name'];
				$data_pd['amount'] = $order_info['order_amount'];
				$data_pd['order_sn'] = $order_info['order_sn'];
				$model_pd->changePd('order_receive',$data_pd);
			}
			
            return callback(true,'操作成功');
        } catch (Exception $e) {
            return callback(false,'操作失败'.$e);
        }
    }
    ...
    public function changeOrderReceivePay($order_list, $role, $user = '', $post = array()) {
        ...
        foreach($order_list as $order_info) {
			...
			if ($order_info['order_state'] != ORDER_STATE_NEW) continue;
            //上述操作并没有解决重复发送的问题,by zjcuntao

            //防止重复发送信息
            $log_condition = array();

            $log_condition['order_id'] = $order_info['order_id'];
            $log_condition['log_orderstate'] = ORDER_STATE_PAY;

            $log_info=$model_order->getOrderLogInfo($log_condition);
            if(!empty($log_info)) continue;
            //end 防止重复发送信息

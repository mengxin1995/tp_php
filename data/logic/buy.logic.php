...
class buyLogic {
    ...
    public function buyStep1($cart_id, $ifcart, $member_id, $store_id) {
        ...
        //计算订单总金额,判断是否达到免运费的要求
        $total_price = 0;

        if(!empty($result['data']['store_goods_total'])){
            foreach ($result['data']['store_goods_total'] as $k=>$v) {
                $total_price +=$v;
            }
        }

        //是否开启跨区域计算运费,并且不满足免运费的要求
        if(C('fee_isuse') && $total_price<C('fee_min')){
            //1.获取收货地址中的city_id
            $city_id = $result['data']['address_info']['city_id'];

            $model_area = Model('area');

            //2.根据city_id,获取地区信息,进而得到province_id
            $condition = array(
                'area_id'   =>$city_id
            );
            $area_info = $model_area->getAreaInfo($condition);

            $member_province_id = $area_info['area_parent_id'];//

            //3.获取店铺的id列表
            foreach ($result['data']['store_cart_list'] as $k=>$v) {
                $store_id_list[] = $k;
            }

            //4.获取店铺所在省份的id,然后将省份id与用户地址的province_id进行拼接,去重.
            if(!empty($store_id_list)){
                $model_store = Model('store');
                foreach ($store_id_list as $store_id) {
                    $store_info = $model_store->getStoreInfoByID($store_id);
                    //$store_province_id_list[] = $store_info['province_id'];
                    $id2id_list[] = $store_info['province_id'].'|'.$member_province_id;
                }
                //5.得到id对应列表
                $id2id_list = array_unique($id2id_list);

                //6.1 根据id对应列表,查找对应的运费商品
                $model_fee = Model('fee');
                foreach ($id2id_list as $id2id) {
                    $fee_info = $model_fee->getFeeById2Id($id2id);
                    if(!empty($fee_info)){
                        $fee_goods_id_list[] = $fee_info['goods_id'];
                    }
                }
            }

            if(!empty($fee_goods_id_list)){
                //6.2 将fee_goods_id_list输出到返回值,
                $result['data']['fee_goods_id_list'] =implode('|', $fee_goods_id_list);

                //7.根据goods_id_list查找所有运费商品信息
                $model_goods = Model('goods');
                foreach ($fee_goods_id_list as $fee_goods_id) {
                    $goods_info = $model_goods->getGoodsInfoByID($fee_goods_id);
                    if(!empty($goods_info)){
                        $fee_goods_list[] = $goods_info;
                    }
                }
            }

            if(!empty($fee_goods_list)){
                //8.将运费商品附加到购物列表
                foreach ($fee_goods_list as $fee_goods) {
                    $result['data']['store_cart_list'][$fee_goods['store_id']][]= array(
                        'cart_id'                   => 0,
                        'buyer_id'                  => $member_id,
                        'store_id'                  => $fee_goods['store_id'],
                        'store_name'                => $fee_goods['store_name'],
                        'goods_id'                  => $fee_goods['goods_id'],
                        'goods_name'                => $fee_goods['goods_name'],
                        'goods_price'               => $fee_goods['goods_price'],
                        'goods_num'                 => '1',
                        'goods_image'               => $fee_goods['goods_image'],
                        'bl_id'                     => '0',
                        'state'                     => true,
                        'storage_state'             => true,
                        'goods_commonid'            => $fee_goods['goods_commonid'],
                        'goods_serial'              => $fee_goods['goods_serial'],
                        'goods_barcode'             => $fee_goods['goods_barcode'],
                        'gc_id'                     => $fee_goods['gc_id'],
                        'transport_id'              => $fee_goods['transport_id'],
                        'goods_freight'             => $fee_goods['goods_freight'],
                        'goods_vat'                 => $fee_goods['goods_vat'],
                        'goods_storage'             => $fee_goods['goods_storage'],
                        'goods_storage_alarm'       => $fee_goods['goods_storage_alarm'],
                        'is_fcode'                  => $fee_goods['is_fcode'],
                        'have_gift'                 => $fee_goods['have_gift'],
                        'groupbuy_info'             => null,
                        'xianshi_info'              => null,
                        'goods_total'               => $fee_goods['goods_price'],
                        'goods_image_url'           => cthumb($fee_goods['goods_image'])
                    );
                    $result['data']['store_goods_total'][$fee_goods['store_id']] += $fee_goods['goods_price'];
                }
            }
        }
        return $result;
    ...
    public function getBuyStep1Data($member_id, $data) {
        ...
        //输出用户默认收货地址,使用最新添加的地址..
        $result['address_info'] = Model('address')->getLastAddressInfo(array('member_id'=>$member_id));
    ...
    private function _createOrderStep1() {
        ...
        //by zjcuntao
        //自提点的地址信息里面，增加站长的姓名和手机号码
        $delivery_model = Model("delivery_point");
        $condition = array();
        $condition["dlyp_id"] = $input_address_info["dlyp_id"];
        $delivery_info = $delivery_model->getDeliveryPointOpenInfo($condition);

        $input_address_info['dlyp_truename']=$delivery_info["dlyp_truename"];
    ...
    private function _createOrderStep2() {
        ...
        if ($post['ifcart']) {
            ...
            if(C('fee_isuse')){
                //处理运费商品
                $fee_goods_id_list = explode('|',$post['fee_goods_id_list']);

                if(!empty($fee_goods_id_list) && $fee_goods_id_list[0]!=''){
                    //7.根据goods_id_list查找所有运费商品信息
                    $model_goods = Model('goods');
                    foreach ($fee_goods_id_list as $fee_goods_id) {
                        $goods_info = $model_goods->getGoodsInfoByID($fee_goods_id);
            ...
            if(C('fee_isuse')){
                ...
                if(!empty($fee_goods_id_list) && $fee_goods_id_list[0]!='') {
                    ...
                    foreach ($fee_goods_list as $fee_goods) {
                        ...
                        $store_cart_list[$fee_goods['store_id']][] = $tmp_goods;
                    }
                }
            }
        }
        ...
        foreach ($store_cart_list as $store_id => $goods_list) {
            ...
            foreach ($goods_list as $goods_info) {
                ...
                if (!intval($goods_info['bl_id'])) {
                    ...
                    $order_goods[$i]['goods_serial'] = $goods_info['goods_serial'];
                    $order_goods[$i]['goods_barcode'] = $goods_info['goods_barcode'];
                    ...
                    foreach ($goods_info['bl_goods_list'] as $bl_goods_info) {
                        ...
                        $order_goods[$i]['goods_serial'] = $bl_goods_info['goods_serial'];
                        $order_goods[$i]['goods_barcode'] = $bl_goods_info['goods_barcode'];
    ...
    private function _createOrderStep5() {
        ...
        //这里直接使用cuntao_pay，优先从充值卡付款，然后从余额付款。
        if (!empty($this->_post_data['cuntao_pay'])) {
    ...
    private function _getGoodsList($cart_list) {
        ...
        foreach ($cart_list as $key => $cart) {
            ...
            if (!intval($cart['bl_id'])) {
                ...
                $goods_list[$i]['goods_serial'] = $cart['goods_serial'];
                $goods_list[$i]['goods_barcode'] = $cart['goods_barcode'];
                ...
                foreach ($cart['bl_goods_list'] as $bl_goods) {
                    ...
                    $goods_list[$i]['goods_serial'] = $bl_goods['goods_serial'];
                    $goods_list[$i]['goods_barcode'] = $bl_goods['goods_barcode'];
